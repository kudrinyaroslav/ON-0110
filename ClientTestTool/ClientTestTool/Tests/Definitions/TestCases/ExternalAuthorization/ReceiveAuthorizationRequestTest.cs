...
namespace ClientTestTool.Tests.Definitions.TestCases.ExternalAuthorization
{
  [Test(
    ...
    Id               = "1",
    FeatureUnderTest = Feature.ReceiveAuthRequest
  )]
  public class ReceiveAuthorizationRequestTest : BaseTest
  {
    ...
    .ToList();
        ...
        var topicExpressionList =
          filterList.Where(item => item.GetRequest<HttpMessage>().ContainsTag("Filter", "TopicExpression"))
            .Where(item =>
            {
              ...
              return null != value && (value.Contains(":AccessControl/Request/Credential") ||
                                       value.Contains(":AccessControl/Request/Anonymous")  ||
                                       value.Contains(":AccessControl/Request//")          ||
                                       value.Contains(":AccessControl//"));
            }).ToList();

        BeginStep(
          "<CreatePullPointSubscription> includes tag <Filter> with tag <TopicExpression> and <TopicExpression>” contains value EITHER (“*:AccessControl/Request/Credential” OR" +
          " “*:AccessControl/Request/Anonymous” OR" +
          " “*:AccessControl/Request//.” OR" +
          " “*:AccessControl//.)",
          topicExpressionList);
      ...
      var validTopicList = topicList.Where(item =>
      {
        var value = TestUtil.ValueOf(item.GetResponse<HttpMessage>(), "Topic");
        return null != value && (value.Contains(":AccessControl/Request/Credential") ||
                                 value.Contains(":AccessControl/Request/Anonymous"));
      }).ToList();

      BeginStep("<NotificationMessage>” includes tag: “<Topic>” with value EITHER (“*:AccessControl/Request/Credential” OR" +
                                                                                  "“*:AccessControl/Request/Anonymous”", topicList, MessageType.Response);
