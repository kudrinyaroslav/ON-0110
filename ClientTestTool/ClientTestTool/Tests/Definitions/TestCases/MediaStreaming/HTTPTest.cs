...
namespace ClientTestTool.Tests.Definitions.TestCases.MediaStreaming
{
  ...
  public class HTTPTest : BaseTest
  {
    protected override void ProcessConversation(Conversation conversation)
    {
      ...
      var httpList = protocolList.Where(item =>
      {
        String value = TestUtil.ValueOf(item.GetRequest<HttpMessage>(), "Protocol");
        return null != value && value.ToUpper().Contains("HTTP");
      }).ToList();

      BeginStep("<Transport> includes tag <Protocol> with \"HTTP\" parameter", httpList);

      if (protocolList.Any() && !httpList.Any())
        throw new TestNotSupportedException("Request <Transport> does not include tag <Protocol> with \"HTTP\" parameter");

      if (!httpList.Any())
        StepFailed("Request <Transport> does not include tag <Protocol> with \"HTTP\" parameter");
