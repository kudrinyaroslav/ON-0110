...
{
  [Test(
    Name             = "Streaming Over RTSP",
    Category         = Category.ProfileS,
    Id               = "3",
    FeatureUnderTest = Feature.RTSPStreaming
  )]
  public class RTSPTest : BaseTest
  {
    protected override void ProcessConversation(Conversation conversation)
    {
      ...
      var rtspList = protocolList.Where(item =>
      {
        String value = TestUtil.ValueOf(item.GetRequest<HttpMessage>(), "Protocol");
        return null != value && value.ToUpper().Contains("RTSP");
      }).ToList();

      BeginStep("<Transport> includes tag <Protocol> with \"RTSP\" parameter", rtspList);

      if (protocolList.Any() && !rtspList.Any())
        throw new TestNotSupportedException("Request <Transport> does not include tag <Protocol> with \"RTSP\" parameter");

      if (!rtspList.Any())
        StepFailed("Request <Transport> does not include tag <Protocol> with \"RTSP\" parameter");
      ...
      rtspList = conversation.GetMessages(ContentType.Rtsp);
