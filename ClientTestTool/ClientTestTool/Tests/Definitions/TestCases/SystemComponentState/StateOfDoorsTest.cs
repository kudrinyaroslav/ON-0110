...
namespace ClientTestTool.Tests.Definitions.TestCases.SystemComponentState
{
  [Test(
    ...
    Id = "2",
    FeatureUnderTest = Feature.StateOfDoors
    )]
  public class StateOfDoorsTest : BaseTest
  {
    ...
    .ToList();
        ...
        var validTopicExpressionList = topicExpressionList.Where(item =>
            {
              ...
              return null != value && (value.Contains(":Door/State") ||
                                       value.Contains(":Door//"));
            }).ToList();

        BeginStep("<CreatePullPointSubscription> includes tag <Filter> with tag <TopicExpression> and (\":Door/State\" OR \":Door//\") topic value", topicExpressionList);

        if (0 == topicExpressionList.Count)
          StepFailed("<TopicExpression> tag is missing");
        else if (0 == validTopicExpressionList.Count)
          throw new TestNotSupportedException("\":AccessPoint/State\" value not found");
      ...
      responseList = pullMessagesList.Where(item => "200" == item.GetResponse<HttpResponse>().StatusCode).ToList();
      ...
      BeginStep("Device response contains <PullMessagesResponse> tag", messageLimitList, MessageType.Response);
