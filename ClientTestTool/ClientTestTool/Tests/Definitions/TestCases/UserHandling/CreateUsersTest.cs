...
namespace ClientTestTool.Tests.Definitions.TestCases.UserHandling
{
  [Test(
    ...
    FeatureUnderTest = Feature.CreateUsers
  )]
  public class CreateUsersTest : BaseTest
  {
    protected override void ProcessConversation(Conversation conversation)
    {
      var filteredList = conversation.GetMessages(ContentType.Http).Where(item => TestUtil.ContainsTag(item.GetRequest<HttpMessage>(), "CreateUsers")).ToList();

      AffectedPairs.AddRange(filteredList);

      //S1
      BeginStep("Client request contains <CreateUsers> tag", filteredList);

      if (0 == filteredList.Count)
        throw new TestNotSupportedException("Conversation does not contain messages with <CreateUsers> tag");

      StepCompleted();

      var userList = filteredList.Where(item => TestUtil.ContainsTag(item.GetRequest<HttpMessage>(), "CreateUsers", "User")).ToList();

      //S2
      BeginStep("<CreateUsers> includes tag: <User>", userList);
      ...
      responseList = responseList.Where(item => TestUtil.ContainsTag(item.GetResponse<HttpMessage>(), "CreateUsersResponse")).ToList();

      //S7
      BeginStep("Device response contains <CreateUsersResponse>", responseList, MessageType.Response);

      if (0 == responseList.Count)
        StepFailed("<CreateUsersResponse> tag is missing");
