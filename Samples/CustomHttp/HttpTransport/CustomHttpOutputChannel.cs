...
namespace Microsoft.ServiceModel.Samples
{
    ...
    /// </summary>
    class CustomHttpOutputChannel : ChannelBase, IOutputChannel
    {
        ...
        CustomHttpChannelFactory parent;
        #endregion

        internal CustomHttpOutputChannel(CustomHttpChannelFactory factory, EndpointAddress remoteAddress, Uri via, MessageEncoder encoder)
            : base(factory)
        {
            // validate addressing arguments
            if (!string.Equals(via.Scheme, CustomHttpConstants.Scheme, StringComparison.InvariantCultureIgnoreCase))
            {
            ...
            //this.socket = new Socket(this.remoteEndPoint.AddressFamily, SocketType.Stream, ProtocolType.Unspecified);
            this.socket = CustomHttp.SocketContainer.Socket;
...
//            }
        ...
        static private byte[] CreateHttpHeaders(long size, string path, string address)
        {
        ...
        class SendAsyncResult : AsyncResult
        {
            ...
            CustomHttpOutputChannel channel;

            public SendAsyncResult(CustomHttpOutputChannel channel, Message message, AsyncCallback callback, object state)
                : base(callback, state)
            {
