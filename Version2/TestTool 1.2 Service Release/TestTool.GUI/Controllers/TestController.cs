...
namespace TestTool.GUI.Controllers
{
    ...
    class TestController : Controller<ITestView>
    {
        ...
        void InternalRun(TestSuiteParameters parameters, bool single)
        {
            ...
            Utils.ManagementServiceProvider provider = new ManagementServiceProvider(parameters.Address, parameters.MessageTimeout);
            provider.ExceptionThrown += new Action<string, Exception>(
                (phase, exc)=>
                    {
                        error = true;
                        View.WriteLine(string.Format("Time synchronization failed: {0}", exc.Message));
                    });
            provider.FaultThrown += new Action<string,System.ServiceModel.FaultException>(
                (phase, exc)=>
                    {
                        error = true;
            ...
            provider.OperationCompleted += new Action(
                () =>
                    {
                        View.WriteLine("Done");
            ...
            View.WriteLine("Synchronize time");
            provider.SyncTime();
        ...
        public void SelectFeatureDependentTests(IEnumerable<Tests.Common.Enums.Feature> features)
        {
            //
            // "Must", "Should", "Optional" tests don't change their state. 
            ...
            foreach (TestInfo testInfo in _testInfos)
            {
                switch (testInfo.RequirementLevel)
                {
                    ...
                    case Tests.Common.Enums.RequirementLevel.ConditionalShould:
                        {
                            bool clear = !Utils.FeaturesHelper.AllFeaturesSelected(testInfo);
                            if (!clear)
                            {
                               tests.Add(testInfo); 
                            }
