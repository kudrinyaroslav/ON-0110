...
namespace TestTool.Tests.TestCases.TestSuites
{
    ...
    class NotificationsTestSuite : EventTest
    {
        ...
        void SetupNotifyServer(Utils.NotifyServer server)
        {
            ...
            server.NotificationReceived += new Action(
                () =>
                    {
                        string content =
                            System.Text.Encoding.UTF8.GetString(server.RawData);
                        try
                        {
            ...
            out string rawSoapPacket)
        {
            ...
            controllers = new IChannelController[] { trafficListener, controller, _semaphore, _credentialsProvider, new SoapValidator(Utils.SoapValidation.EventsSchemasSet.GetInstance()) };
        ...
        void ValidateNotificationsPacket(byte[] data)
        {
            BeginStep("Validate notifications SOAP packet");
            MemoryStream stream = new MemoryStream(data);
            SoapValidator validator = new SoapValidator(Utils.SoapValidation.EventsSchemasSet.GetInstance());
            try
            {
        ...
        XmlDocument LoadNotificationsAsXml(byte[] data)
        {
