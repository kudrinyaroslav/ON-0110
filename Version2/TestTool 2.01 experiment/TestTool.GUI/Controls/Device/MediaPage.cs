...
{
    partial class MediaPage : UserControl, IMediaView
    {
        ...
        public void SetProfiles(Media.Profile[] profiles)
        {
            BeginInvoke(new Action(() =>
            {
                ...
                if (testProfile == null)
                {
                    testProfile = new MediaProfileWrapper() { Profile = new TestTool.Proxies.Media.Profile() };
        ...
        public MediaPage()
        {
            ...
            cmbTransport.Items.Add(Media.TransportProtocol.UDP);
            cmbTransport.Items.Add(Media.TransportProtocol.TCP);
            cmbTransport.Items.Add(Media.TransportProtocol.HTTP);
        ...
        private void buttonGetVideoCodecs_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "Select video encoder configuration", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                try
        ...
        private void buttonGetAudioCodecs_Click(object sender, EventArgs e)
        {
            ...
            if (encoderConfig == null)
            {
                MessageBox.Show(this, "Select audio encoder configuration", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                try
        ...
        private void txtVideoBitrate_Validating(object sender, CancelEventArgs e)
        {
            if(!string.IsNullOrEmpty(txtVideoBitrate.Text))
            {
                ...
                if (!valid)
                {
                    MessageBox.Show(this, "Please enter integer value", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    e.Cancel = true;
                }
            }
        }
