...
namespace TestTool.GUI.Controls
{
    ...
    partial class SetupPage : UserControl, ISetupView
    {
        ...
        /// </summary>
        public CoreSpecification CoreSpecification
        {
            get
            {
                return _versions[cmbCoreVersion.SelectedIndex];
            }
            set
            {
                for (int i=0; i< cmbCoreVersion.Items.Count; i++)
                {
                    if (_versions[i] == value)
                    {
                        cmbCoreVersion.SelectedIndex = i;
                        break;
                    }
                }
            }
        ...
        private List<CoreSpecification> _versions;

        /// <summary>
        /// Enables/disables specification selection when certification/diagnostics mode is selected.
        /// </summary>
        /// <param name="enable"></param>
        public void EnableSpecificationSelection(bool enable)
        {
            cmbCoreVersion.Enabled = enable;
        }
    }
}
