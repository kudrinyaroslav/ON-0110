...
namespace TestTool.GUI.Controls
{
    partial class TestPage : UserControl, ITestView
    {
        ...
        private Dictionary<string, string> _groupNumbers;
        ...
        string FindImageKey(RequirementLevel level,
            bool supported,
            bool implemented)
        {
        ...
        TreeNode FindGroupNode(string path)
        {
...
#if DEBUG
                rootNode.ToolTipText = string.Format("path: [{0}], number: [{1}]", rootName, (3 + tvTestCases.Nodes.Count));
#endif

                int groupNumber = (FIRSTGROUPNUMBER - 1 + tvTestCases.Nodes.Count);
 
                _groupNumbers.Add(rootName, groupNumber.ToString());
            }
            ...
            else
            {
                ...
                string currentNumber = _groupNumbers[parts[0]];
                ...
                for (int i = 1; i < parts.Length; i++)
                {
                    ...
                    if (next == null)
                    {
                        currentNumber += "." + (current.Nodes.Count + 1).ToString();
                        next = current.Nodes.Add(group);
                        _groupNodes.Add(next);

                        _groupNumbers.Add(currentPath, currentNumber);
#if DEBUG
                        next.ToolTipText = string.Format("path: [{0}], number: [{1}]", currentPath, currentNumber);
#endif
                        next.Name = group;
                    }
                    else
                    {
                        currentNumber = _groupNumbers[currentPath];
                    }
                    current = next;
                }
        ...
        void UpdatePauseButton()
        {
            if (_controller.State == TestState.Paused)
            {
                ...
                tsbPause.ToolTipText = "Continue tests execution";
                tsbPause.Image = Properties.Resources.RunSelected;
            }
            else
            {
                tsbPause.Text = "Pause";
                tsbPause.ToolTipText = "Pause tests execution at IO operation";
        ...
        /// <summary>
        /// Clears test execution information.
        /// </summary>
        void ClearTestInfo()
        {
        ...
        /// Selectes "must" and "should" tests depending on services selection.
        /// </summary>
        /// <param name="testInfos">Tests to be selected.</param>
