...
namespace TestTool.Tests.TestCases.TestSuites
{
    ...
    public class DeviceManagementIPTestSuite : Base.DeviceDiscoveryTest
    {
        ...
        void SetIPConfigurationInt(string interfaceToken, NetworkInterfaceSetConfiguration configuration, bool restore)
        {
            bool rebootNeeded = false;
            SoapMessage<WSD.HelloType> hello = null;
            try {
                hello = ReceiveHelloMessage(false, true, new Action(() =>
                {
            ...
            RunNormal("Verifying Hello message", new Action(() =>
            {
                MyAssert(
                    ...
                    LogLog("Service Address(es) = " + hello.Object.XAddrs)
                    , "Hello not verified", "Hello verified successfully");
            }));
        ...
        void SetIPConfiguration(string interfaceToken, NetworkInterfaceSetConfiguration configuration)
        {
            SetIPConfigurationInt(interfaceToken, configuration, false);
        }
 
        void RestoreNetworkInterface(string interfaceToken, NetworkInterface networkInterface)
        {
...
#else
                configuration.IPv4.Manual = new PrefixedIPv4Address[] { new PrefixedIPv4Address() {Address = "192.168.10.201", PrefixLength = 16 } };
                SetIPConfiguration(token, configuration);
...
#else
                        LogAssert(
                            zeroConf.Addresses.Where(NI => NI.Equals(modified.IPv4.Config.LinkLocal.Address)).Count() > 0, 
                            "No IP address in addresses set") &&
                        LogAssert(
                            zeroConf.Addresses.Where(NI => NI.Equals(modified.IPv4.Config.LinkLocal.Address)).Count() != 1, 
                            "More than one equal IP address in addresses set")
#endif
#else
        ...
        bool TestIPv6InterfaceByFeature(bool IPv6Feature, string interfaceToken)
        {
            ...
            if (!IPv6Feature)
            {
                EndTest(Tests.Common.Trace.TestStatus.NotSupported);
                return true;
            }
            return false;
        }
...
/*                        BackupConnection();
                    ...
                    () =>
                    {
                        ...
                        configuration.IPv4.EnabledSpecified = true;
                        configuration.IPv4.DHCP = false;
                        configuration.IPv4.DHCPSpecified = false;
                        ...
                        Assert(modified != null,
                            string.Format("Interfaces with token = '{0}' not found", token),
                            string.Format("Check if an interfaces with token = '{0}' is presented", token));
...
/*                        BackupConnection();
                    ...
                    () =>
                    {
                        ...
                        configuration.IPv6.Enabled = true;
                        configuration.IPv6.EnabledSpecified = true;
                        configuration.IPv6.AcceptRouterAdvert = false;
                        configuration.IPv6.AcceptRouterAdvertSpecified = true;
                        configuration.IPv6.DHCP = IPv6DHCPConfiguration.Off;
                        configuration.IPv6.DHCPSpecified = true;
                        ...
                        foreach (NetworkInterface n in originalSettings)
                        {
                            ...
                            configuration.IPv6.Manual =
                                new PrefixedIPv6Address[] { 
                                    new PrefixedIPv6Address() { Address = "2001:1:1:1:1:1:1:1" } 
                                };
                            if ((n.IPv6.Config != null) && n.IPv6.Config.AcceptRouterAdvertSpecified)
                            {
                                configuration.IPv6.AcceptRouterAdvert = n.IPv6.Config.AcceptRouterAdvert;
                        ...
                        Assert(token != null,
                            "No appropriate interface found",
                            "Check if an interface with IPv6 configuration is presented");

                        SetIPConfiguration(token, configuration);
                        

                        NetworkInterface[] actualInterfaces = GetNetworkInterfaces();

                        Assert(actualInterfaces != null,
                            "The DUT did not return Network Interfaces",
                            "Check if Network Interfaces returned from the DUT");

                        NetworkInterface modified =
                            actualInterfaces.Where(ni => ni.token == token).FirstOrDefault();
