...
{        
    ...
    partial class PacsEventsTestSuite : NotificationsTestSuite
    {
            ...
            Dictionary<NotificationMessageHolderType, XmlElement> messages, out DateTime subscribeStarted)
        {
            ...
            Utils.NotifyCollectingServer server = new NotifyCollectingServer(_nic);
        ...
        protected Dictionary<NotificationMessageHolderType, XmlElement> 
            GetFilteredList(Dictionary<NotificationMessageHolderType, XmlElement> messages,
            Func<NotificationMessageHolderType, bool> messageCheck)
        {
            Dictionary<NotificationMessageHolderType, XmlElement> filtered = new Dictionary<NotificationMessageHolderType, XmlElement>();
            foreach (NotificationMessageHolderType m in messages.Keys)
            {
                if (messageCheck(m))
                {
                    filtered.Add(m, messages[m]);
                }
            }
            ...
            TopicInfo topicInfo, StringBuilder logger, string sourceSimpleItemName)
        {
            ...
            else
            {
                ...
                if (success)
                {
                    bool localOk = ValidateSimpleItemDescription(sourceSimpleItems, "Source",
                        sourceSimpleItemName, "ReferenceToken", PTNAMESPACE, logger);
                    ok = ok && localOk;
                }
                else 
                {
        ...
        protected bool ValidateSimpleItemDescription(Dictionary<string, XmlElement> sourceSimpleItems, 
            string part,
            string itemName, string itemType, string typeNamespace, StringBuilder logger)
        {
            bool ok = true;
            ...
            {
                logger.AppendFormat("'{0}' SimpleItemDescription is missing in {1}{2}", itemName, part, Environment.NewLine);
                ok = false;
            }
        ...
        {
            {
                ...
                foreach (NotificationMessageHolderType message in messages)
                {
                    ...
                    {
                        ok = false;
                        logger.AppendLine(string.Format("Notification for token='{0}' was sent more that once", entityToken, Environment.NewLine));
                    }
                    else
                    {
