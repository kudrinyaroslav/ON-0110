...
namespace TestTool.Tests.TestCases
{
    ...
    static class BaseNotificationUtils
    {
        ...
        /// <returns></returns>
        private static Dictionary<string, string> GetMessageSimpleItems(string part, XmlElement message)
        {
            Dictionary<string, string> items = new Dictionary<string, string>();
        ...
        public static bool IsCorrectQName(this XmlAttribute attr, 
            ...
            XmlElement element, ref string error)
        {
            bool ok = true;
            ...
            if (parts.Length == 2)
            {
                string ns = element.GetNamespaceOfPrefix(parts[0]);
                ...
                else
                {
                    ...
                    {
                        error = string.Format("expected '{0}' from namespace '{1}', actual '{2}' from namespace '{3}'",
                            expectedLocal, expectedNamespace, parts[1], ns);
                        ok = false;
                    }
                }
            }
