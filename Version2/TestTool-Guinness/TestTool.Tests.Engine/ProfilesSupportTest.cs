...
namespace TestTool.Tests.Engine
{
    class ProfilesSupportTest
    {
        ...
        internal void CheckProfiles(IEnumerable<IProfileDefinition> profiles, IEnumerable<Feature> features, IEnumerable<string> scopes)
        {
            foreach (IProfileDefinition profile in profiles)
            {
                string dump = string.Empty;
                ProfileStatus status = profile.Check(out dump, features, scopes);
