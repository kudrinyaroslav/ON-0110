...
namespace TestTool.Tests.TestCases.Base
{
    public class PTZTest : BaseServiceTest<PTZ, PTZClient>
    {
        ...
        protected Profile[] GetProfiles()
        {
            ...
            RunStep(() => { profiles = media.GetProfiles(); }, "Get Media Profiles");
            return profiles;
        }
        ...
        /*
        protected Profile GetPTZProfile(PTZNode[] nodes, 
            Func<PTZNode, bool> nodesSelector, out PTZConfigurationOptions options)
        {
            List<string> nodeTokens = new List<string>();

            // select nodes
            BeginStep("Select appropriate PTZ nodes");
            foreach (PTZNode node in nodes)
            {
                if (nodesSelector(node))
                {
                    nodeTokens.Add(node.token);
                    LogStepEvent(node.token);
                }
            }
            if (nodeTokens.Count == 0)
            {
                new AssertException("No appropriate nodes were found");
            }
            StepPassed();
            ...
            Assert(!string.IsNullOrEmpty(_mediaAddress), "Media service address not found",
                   "Check that the DUT returned media service address");
            ...
            Assert(profiles != null, "No profiles returned", "Check that the DUT returned at least one Media profile");
            ...
            foreach (Profile profile in profiles)
            {
                if (profile.PTZConfiguration != null)
                {
                    if (nodeTokens.Contains(profile.PTZConfiguration.NodeToken))
                    {
                        res = profile;
                        break;
            ...
            if (res == null)
            {
                ...
                res =
                    profiles.Where(p => p.VideoSourceConfiguration != null && p.VideoEncoderConfiguration != null).
                        FirstOrDefault();
                ...
                Assert(res != null, "Profile with VideoEncoderConfiguration and VideoSourceConfiguration not found");
                ...
                RunStep(() =>
                {
                    ...
                    PTZConfiguration config = configurations.FirstOrDefault(c => nodeTokens.Contains(c.NodeToken) );
                    if (config != null)
                    {
                    ...
                    {
                        media.Close();
                        throw new AssertException("No PTZ configuration found for node with appropriate properties");
                    }
                }, string.Format("Add PTZ Configuration"));
                DoRequestDelay();
            ...
            {
                LogTestEvent(string.Format("Use profile with token {0} for testing", res.token));
            }
        ...
        */
        ...
        {
            CheckPTZPosition(position, pantilt, zoom, false);
        }
        ...
        /// <param name="compareSpaces">True, if spaces needs to be compared</param>
        ...
        protected void CheckPTZPosition(PTZVector position, PTZVector pantilt, PTZVector zoom, bool compareSpaces)
        {
            RunStep(() =>
            {
        ...
        /// <remarks>CheckPreset now is used in two places: 
        /// RemovePreset - here null is passed as position
        /// SetGetPreset - here spaces comparison is not needed.
        /// So, CheckPTZPosition is called with "false" last argument
        /// </remarks>
        protected void CheckPreset(PTZPreset[] presets, string presetToken, string name, PTZVector position)
        {
            ...
            if ((preset.PTZPosition != null) && (position != null))
            {
                ...
                CheckPTZPosition(preset.PTZPosition, position, position, false);
            }
        }
