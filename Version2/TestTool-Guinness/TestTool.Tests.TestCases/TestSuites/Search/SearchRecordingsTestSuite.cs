            ...
            RunTest(
                () =>
                {
                    ...
                    searchToken = FindRecordings(
                        new SearchScope(){RecordingInformationFilter = "boolean(//Track[TrackType = \"Video\"])"}, 
                        null, 
                        keepAlive);
                    ...
                    foreach (RecordingInformation recording in recordings)
                    {
                        ...
                        if (recording.Track != null)
                        {
                            ...
                            {
                                if (info.TrackType == TrackType.Video)
                                {
                                    found = true;
                                    break;
                                }
                        ...
                        if (!localOk)
                        {
                            logger.AppendFormat("Recording {0} does not contain tracks of type Video{1}", recording.RecordingToken, Environment.NewLine);
                            ok = false;
                        }
                    }

                    Assert(ok, logger.ToStringTrimNewLine(),
                           "Check that all recordings have Video tracks");
