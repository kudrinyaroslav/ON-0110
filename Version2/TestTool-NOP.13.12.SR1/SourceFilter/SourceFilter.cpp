...
void StartCounter()
{
  ...
  PCFreq = 0.000001 * (double)li.QuadPart;
...
{
  static REFERENCE_TIME Correction = 0;
  Tick++;
  if (!BackIn)
  {
    Correction = 0;
  ...
  REFERENCE_TIME SlicePrev = CurrentIn - BackIn;
  BackIn = CurrentIn;
  LiveCapsule *Capsule = ((CRTSPSource*)m_pFilter)->GetCapsule();
  CurrentIn = Current + (Capsule->ReplayReverse() && !Audio ? Correction : 0);
  if (CurrentIn >= BackIn)
  {
  ...
  else
  {
    ...
    if (Capsule->Replay()) {
      Correction += BackIn - CurrentIn + SlicePrev;
      Slice = SlicePrev;
    }
  }
  if (SliceAverage) 
  {
    // use adaptive algorithm after 10 frames history, use fixed algorithm for very big gaps (more than 5 seconds)
    if (((Slice > SliceAverage * 4) && (Tick > 10)) || (Slice > 50000000))
    {
...
HRESULT CRTSPPinMedia::FillBuffer(IMediaSample *pSample)
{
	...
	//[11.03.2014] AKS: avoid buffer overrun if nSize < fi->frameHead.FrameLen
    //memcpy(pData, fi->pdata, fi->frameHead.FrameLen);
	memcpy(pData, fi->pdata, nSize);

    pSample->SetActualDataLength(nSize);

    // manual frame times in reverse mode
    static double prevClock = GetCounter();
    if (Capsule->ReplayReverse() && !Audio)
    {
      double currClock = GetCounter();
      Slice = (REFERENCE_TIME)(10.0 * (currClock - prevClock));
      prevClock = currClock;
    }
    else
    {
    ...
    DropDebug("Get %i, size(%i), time(%lu), slice(%lu)\n",
    Audio,
    nSize,
...
{
  OutputDebugStringA("RTSPSource Destructor");

  if (Capsule.IsAlive()) {
    OutputDebugStringA("Capsule still alive");
    ControlStream(2);
    Sleep(2000);
    ControlStream(0);
    Sleep(1000);
    if (Capsule.IsDead()) {
      OutputDebugStringA("Capsule killed");
    } else {
      OutputDebugStringA("Capsule NOT killed");
    }
  } else {
    OutputDebugStringA("Capsule dead");
  }
...
STDMETHODIMP CRTSPSource::ControlStream(int Control)
{
 try {
  ...
  case 1:
    {
      ...
      if (Capsule.GetState() != RTSP_STATE_PLAYING)
      {
        if (Ret || (Capsule.GetState() == RTSP_STATE_OPENED)) {
          Capsule.StopStream();
        }
        return E_FAIL;
      }
  ...
  case 3:
    {
      OutputDebugStringA("CRTSPSource::PauseStream");
      Capsule.PauseStream();
      return S_OK;
    }
  case 4:
...
{
  OutputDebugStringA("####FILTER OnThreadCreate");
  return NOERROR;
}

HRESULT CRTSPPinMedia::OnThreadDestroy(void)
{
  OutputDebugStringA("####FILTER OnThreadDestroy");
  return NOERROR;
}

HRESULT CRTSPPinMedia::OnThreadStartPlay(void)
{
  OutputDebugStringA("####FILTER OnThreadStartPlay");
  return NOERROR;
}

HRESULT CRTSPPinMedia::DoBufferProcessingLoop()
{ 
  OutputDebugStringA("####FILTER DoBufferProcessingLoop begin");
  HRESULT Res = CSourceStream::DoBufferProcessingLoop();
  OutputDebugStringA("####FILTER DoBufferProcessingLoop end");
  return Res;
}
