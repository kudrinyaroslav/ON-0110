...
//                            out notifications);
                        ...
                        Timeout = !Handler.WaitMessages(1, pullingCondition, out notifications);
                    }
                    finally
                    {
            ...
            string stateSimpleItemName)
        {
            ...
            RunTest(
                () =>
                {
                    ...
                    {
                        NotificationMessage = ReceiveMessagesPullPointFirstMessage(new Action(() => { }), filter.Filter, 1, doc);
                    }
                ...
                () =>
                {
                    ...
                    try
                    {
                        subscriptionReference =
                            ...
                            new Action(() =>
                            {
                                Operator.ShowDoorSelectionMessage(data);
                            }),
                            1,
                            (n) => CheckMessagePropertyOperation(n, OnvifMessage.CHANGED),
                            notifications,
                    ...
                    else
                    {
                        try
                        {
                            NotificationMessage = ReceiveMessagesPullPointFirstMessage(
                                new Action(() =>
                                {
                                    Operator.ShowDoorSelectionMessage(data);
                                }),
                    ...
                    XmlNamespaceManager manager1 = CreateNamespaceManager(doc);
                    Dictionary<NotificationMessageHolderType, XmlElement> notifications = GetRawElements(NotificationMessage, doc, manager1, UseNotify);
#endif
...
/*
        ...
        public void DoorControlDoorModeEventTestBis()
        {
...
/*
        [Test(Name = "DOOR CONTROL – DOOR PHYSICAL STATE EVENT STATE CHANGE",
            ...
            RequirementLevel = RequirementLevel.Optional,
            FunctionalityUnderTest = new Functionality[] { Functionality.Notify, Functionality.TopicFilter, Functionality.DoorPhysicalStateEvent })]
        public void DoorControlDoorPhysicalStateEventStateChangeTestBis()
        {
            Func<DoorInfo, bool> doorCapabilitiesTest =
                D => D.Capabilities != null &&
                    D.Capabilities.DoorMonitorSpecified &&
