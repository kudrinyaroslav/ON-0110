...
namespace TestTool.Tests.TestCases.TestSuites.Events
{
    ...
    class EventsProxy
    {
        ...
        void Decorate<TChannel>(
            System.ServiceModel.ClientBase<TChannel> Client, 
            bool Addressing
            ) where TChannel : class
        {
            ...
            Client.InnerChannel.OperationTimeout = new TimeSpan(0, 0, 0, 0, baseTest._messageTimeout);
        }
        void DecorateTimeout<TChannel>(
            System.ServiceModel.ClientBase<TChannel> Client,
            string Timeout
            ) where TChannel : class
        {
            int LocTimeout = Convert.ToInt32(Timeout.DurationToSeconds() * 1000);
            if (LocTimeout <= 0)
            {
                LocTimeout = baseTest._messageTimeout;
            }
            else
            {
                LocTimeout += 1000; // add one second
            }
            Client.InnerChannel.OperationTimeout = new TimeSpan(0, 0, 0, 0, LocTimeout);
        }
        ...
        {
            baseTest.LogStepEvent(string.Format(
                "{0}Exception of type FaultException<UnacceptableInitialTerminationTimeFaultType> received. Will try to Subscribe with new parameters{0}", Environment.NewLine));
            baseTest.StepPassed();
        ...
        public EndpointReferenceType CreatePullPointSubscription(
            FilterType Filter,
            string InitialTerminationTime,
            out DateTime CurrentTime,
            out Nullable<DateTime> TerminationTime
            )
        {
            ...
            try
            {
                ...
                () =>
                {
                    result = eventPortTypeClient.CreatePullPointSubscription(
                        Filter,
            ...
            catch (FaultException<UnacceptableInitialTerminationTimeFaultType> ex)
            {
                ...
                () =>
                {
                    result = eventPortTypeClient.CreatePullPointSubscription(
                        ...
                        out currentTimeCopy,
                        out terminationTimeCopy);
                }, "Create Pull Point Subsciption with another time");
            }
        ...
        {
            ValidatePullPointSubscription("PT0S");
        ...
        {
            ValidatePullPointSubscription(Timeout);
            ...
            () =>
            {
                response = pullPointSubscriptionClient.PullMessages(Timeout, MessageLimit, null, out locTerminationTime, out locNotificationMessage);
            ...
            )
        {
            ...
            ValidatePullPointSubscriptionLocal(Timeout);
            
            string localDump = null;
            Action<string> OnReceive = (data) =>
                {
                    localDump = data;
                };
            ...
            Action PullAction = () =>
                {
                    try
                    {
                        response = pullPointSubscriptionClientLocal.PullMessages(Timeout, MessageLimit, null, out locTerminationTime, out locNotificationMessage);
                    }
                    catch (Exception)
                    {
    ...
    class SubscriptionHandler
    {
        ...
        public DateTime? TerminationTime { get { return m_TerminationTime; } }

        private DateTime m_CurrentTime;
        public DateTime CurrentTime { get { return m_CurrentTime; } }

        public string PullTimeout { get; set; }
        ...
        public void Subscribe(FilterType Filter, int TimeoutSeconds)
        {
            ...
            {
                Subscription = Proxy.CreatePullPointSubscription(
                    Filter,
                    InitialTerminationTime,
                    out m_CurrentTime,
                    out m_TerminationTime);
            ...
            EventServiceUtils.ValidateSubscription(TerminationTime,
                CurrentTime,
                TimeoutSeconds,
                Subscription,
                baseTest.Assert);

            SubscriptionDeadline = TerminationTime.HasValue ?
                TerminationTime.Value + DUTTimeShift : 
                SubscriptionStarted.AddSeconds(TimeoutSeconds);
        ...
        public void GetMessages(
            int Count, 
            out NotificationMessageHolderType[] NotificationMessages
            )
        {
            DateTime Current = DateTime.MinValue;
            DateTime LocalSubscriptionDeadline;
            if (Notify)
            {
                NotificationMessages = null;
                LocalSubscriptionDeadline = SubscriptionDeadline - DUTTimeShift;
            }
            else
            {
                bool UseRetry = false;
ReplayPullMessages:
                ...
                try
                {
                    Current = Proxy.PullMessages(
                        PullTimeout,
                        Count,
                        out LocalSubscriptionDeadline,
                        out NotificationMessages);
                }
                catch (FaultException<PullMessagesFaultResponseType> fault)
                {
                    baseTest.StepPassed();
                    baseTest.LogStepEvent(string.Format(
                        "Exception of type FaultException<PullMessagesFaultResponseType> received. Will try to pull messages with new parameters{0}", Environment.NewLine));
                    ...
                    PullTimeout = fault.Detail.MaxTimeout;
            ...
            SubscriptionDeadline = LocalSubscriptionDeadline + DUTTimeShift;
            ValidateMessages(
                Count,
                NotificationMessages,
                "",
                LocalSubscriptionDeadline,
                Current);
        }
...
ReplayPullMessages:
            ...
            )
        {
            ...
            if (Notify)
            {
                Notify notify = Server.WaitForNotifyOnly(SomethingInTheMiddle, 
                    (int)(PullTimeout.DurationToSeconds() * 1000),
                    baseTest.Semaphore.StopEvent);
...
ReplayPullMessages:
        ...
        void AutoRenew()
        {
            ...
            DateTime LocalDeadline = DateTime.Now.AddSeconds(PullTimeout.DurationToSeconds() + 2);
...
repeatRenew:
            ...
            catch (FaultException<UnacceptableTerminationTimeFaultType> exc)
            {
                ...
                if (diffSeconds < PullTimeout.DurationToSeconds())
            ...
            )
        {
            ...
            {
                baseTest.Assert(NotificationMessages != null && NotificationMessages.Length > 0,
