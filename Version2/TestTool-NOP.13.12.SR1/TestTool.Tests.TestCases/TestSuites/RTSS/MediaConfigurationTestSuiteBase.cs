    ...
    class MediaConfigurationTestSuiteBase : RTSSTestSuite
    {
            ...
            MediaConfigurationChangeLog changeLog)
        {
            ...
            if (profile == null)
            {
                ...
                profile = CreateProfileHandleMaxNvtProfiles(TESTPROFILENAME,
                    nonFixedProfile, oldProfile, false, out isNew, changeLog);
            ...
            MediaConfigurationChangeLog changeLog)
        {
            ...
            if (profile == null)
            {
                ...
                // from the DUT.
                bool isNew;
                profile = CreateProfileHandleMaxNvtProfiles(TESTPROFILENAME, nonFixedProfile, oldProfile, false, out isNew, changeLog);
            ...
            MediaConfigurationChangeLog changeLog)
        {
            ...
            if (profile == null)
            {
                string testProfileName = "TestProfile1";
                bool isNew = false;
                Profile newProfile = CreateProfileHandleMaxNvtProfiles(testProfileName, null, null, true, out isNew, changeLog);
                if (!isNew)
                {
            ...
            MediaConfigurationChangeLog changeLog)
        {
            ...
            if (profiles.Count < totalNumber)
            {
                ...
                while (true)
                {
                    ...
                    if (idx < profilesToDelete.Count)
                    {
                        nextProfileToBeDeleted = profilesToDelete[idx];
                    }

                    if (idxUpdate < profilesToUpdate.Count)
                    {
                        nextProfileToBeUpdated = profilesToUpdate[idxUpdate];
                    }

                    bool isNew;
                    string profileName = string.Format("TestProfile{0}", changeLog.LastProfileNumberUsed + 1);
                    Profile newProfile = CreateProfileHandleMaxNvtProfiles(profileName, nextProfileToBeDeleted, nextProfileToBeUpdated, false, out isNew, changeLog);
                    ...
                    // if there was no need to delete profile, null as nextProfileToBeDeleted is OK for 
                    // CreateProfileHandleMaxNvtProfiles
                    if (nextProfileToBeDeleted != null && changeLog.DeletedProfiles.FirstOrDefault(P => P.token == nextProfileToBeDeleted.token) != null)
                    {
                        idx++;
                    }
            ...
            MediaConfigurationChangeLog changeLog)
        {
...

            {
                ...
                bool isNew;
                profile = CreateProfileHandleMaxNvtProfiles(profileName, nonFixedProfile, profileForUpdate, false, out isNew, changeLog);
                if (!isNew)
                {
                    Profile backup = Utils.CopyMaker.CreateCopy(profile);
                    changeLog.TrackModifiedProfile(backup);
