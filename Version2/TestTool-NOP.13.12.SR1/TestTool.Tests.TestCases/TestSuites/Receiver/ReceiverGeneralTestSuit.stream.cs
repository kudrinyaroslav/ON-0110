...
namespace TestTool.Tests.TestCases.TestSuites
{
    ...
    class ReceiverEventsGeneralTestSuit : NotificationsTestSuite
    {
        ...
        public ReceiverEventsGeneralTestSuit(TestLaunchParam param)
            : base(param)
        {
                ...
                () => 
                    {
...

                        {
                            ...
                            LogTestEvent(string.Format("Send PullMessages requests until an event with 'ReceiverToken' Simple Item with value='{0}' is received{1}",
                                token, Environment.NewLine));

                            Func<Event.NotificationMessageHolderType, bool> eventCheck =
                            (m) => { return CheckConnectionFailedMessage(m, token); };

                            Dictionary<Event.NotificationMessageHolderType, XmlElement> notifications =
                                GetMessages(subscriptionReference, T1, eventCheck);
        ...
        void ConfigureReceiverTest(StreamType type, TransportProtocol protocol)
        {
            ...
            RunTest(() =>
            {
                ...
                // Get and validate notifications
                {
                    ...
                    LogTestEvent(string.Format("Send PullMessages requests until an event with 'ReceiverToken' Simple Item with value='{0}' and 'NewState' Simple Item with value='{1}' is received{2}", 
                        token, "Connected", Environment.NewLine));

                    Func<Event.NotificationMessageHolderType, bool> eventCheck =
                    (m) => { return CheckReceiverChangedMessage(m, token, ReceiverState.Connected ); };
...

                {
                    ...
                    LogTestEvent(string.Format("Send PullMessages requests until an event with 'ReceiverToken' Simple Item with value='{0}' and 'NewState' Simple Item with value='{1}' is received{2}",
                        token, "NotConnected", Environment.NewLine));

                    Func<Event.NotificationMessageHolderType, bool> eventCheck =
                    (m) => { return CheckReceiverChangedMessage(m, token, ReceiverState.NotConnected); };
            ...
            ReceiverMode expectedMode)
        {
            ...
            foreach (Proxies.Event.NotificationMessageHolderType message in notifications.Keys)
            {
                ...
                if (ok)
                {
                    ...
                    Dictionary<string, string> dataSimpleItems =
                        BaseNotificationUtils.GetMessageSimpleItems(OnvifMessage.DATA, message.Message);
