        ...
        private void CheckRecordingListChanged(GetRecordingsResponseItem[] prevList, GetRecordingsResponseItem[] newList)
        {
            ...
            if (prevList != null && newList != null)
            {
                foreach (var item in newList)
                {
                    ...
                    {
                        ok = false;
                        logger.Append(string.Format("First recording list doesn't contain recording with token '{0}'{1}", item.RecordingToken, Environment.NewLine));
                    }
                    else
                    {
                        ok &= CompareRecordings(item, recording, logger);
                    }
                }
            }
            else
            {
                ...
                if (prevList == null && newList != null && newList.Length > 0)
                {
                    ok = false;
