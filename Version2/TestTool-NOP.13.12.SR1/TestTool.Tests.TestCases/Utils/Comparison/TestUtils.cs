...
namespace TestTool.Tests.CommonUtils.Comparison
{
    ...
    public static class TestUtils
    {
        ...
        /// <param name="dump">Log</param>
        /// <returns></returns>
        public static bool BatchCheckBooleanAllowMissing<T>(T s1, T s2,
            string descr1, string descr2,
            List<CheckSettings<T>> batch, string structureName, StringBuilder dump)
        {
            bool equal = true;
            ...
            foreach (CheckSettings<T> settings in batch)
            {
                bool local = CheckBooleanFieldHandleNull(s1, s2, settings.ValueSelector, settings.SpecifiedSelector,
                    string.IsNullOrEmpty(structureName) ? settings.FieldName : string.Format("{0}.{1}", structureName, settings.FieldName),
                    descr1, descr2, dump);
        ...
        /// <param name="dump">Log</param>
        /// <returns></returns>
        public static bool CheckBooleanFieldHandleNull<T>(T s1, T s2,
            Func<T, bool> valueSelector, Func<T, bool> specifiedSelector,
            string name, string descr1, string descr2, StringBuilder dump)
        {
            bool specified1 = s1 == null ? false : specifiedSelector(s1);
            bool value1 = specified1 ? valueSelector(s1) : false;
            bool specified2 = s2 == null ? false : specifiedSelector(s2);
            bool value2 = specified2 ? valueSelector(s2) : false;
            
            return CheckField(specified1, value1,
                specified2, value2,
                name, descr1, descr2, dump);
        }
        ...
        public static bool CheckIntField<T>(T s1, T s2,
            Func<T, int> valueSelector, Func<T, bool> specifiedSelector,
            string fieldName, string descr1, string descr2, StringBuilder dump)
        {
            bool ok = false;
