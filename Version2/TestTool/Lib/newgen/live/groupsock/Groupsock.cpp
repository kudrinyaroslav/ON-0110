...
OutputSocket::OutputSocket(UsageEnvironment& env, int af)
  : Socket(env, 0 /* let kernel choose port */, af),
...
Boolean OutputSocket::write(NetAddress const& address, Port port, u_int8_t ttl,
			    unsigned char* buffer, unsigned bufferSize) {
  if ((unsigned)ttl == fLastSentTTL) {
    // Optimization: Don't do a 'set TTL' system call again
    if (!writeSocket(env(), socketNum(), address, port, buffer, bufferSize)) return False;
  } else {
    if (!writeSocket(env(), socketNum(), address, port, ttl, buffer, bufferSize)) return False;
...
destRecord
::destRecord(NetAddress const& addr, Port const& port, u_int8_t ttl,
	     destRecord* next)
  : fNext(next), fGroupEId(addr, port.num(), Scope(ttl)), fPort(port) {
}
...
Groupsock::Groupsock(UsageEnvironment& env, NetAddress const& groupAddr,
		     Port port, u_int8_t ttl)
  : OutputSocket(env, port, groupAddr.af()),
    deleteIfNoMembers(False), isSlave(False),
    fIncomingGroupEId(groupAddr, port.num(), Scope(ttl)), fDests(NULL), fTTL(ttl) {
...
Groupsock::Groupsock(UsageEnvironment& env, NetAddress const& groupAddr,
  ...
  if (!socketJoinGroupSSM(env, socketNum(), ((struct in_addr)groupAddr).s_addr,
			  ((struct in_addr)sourceFilterAddr).s_addr)) {
    if (DebugLevel >= 3) {
...
Groupsock::~Groupsock() {
  if (isSSM()) {
    if (!socketLeaveGroupSSM(env(), socketNum(), ((struct in_addr)groupAddress()).s_addr,
			     ((struct in_addr)sourceFilterAddress()).s_addr)) {
...
void
Groupsock::changeDestinationParameters(NetAddress const& newDestAddr,
				       Port newDestPort, int newDestTTL) {
  if (fDests == NULL) return;

  NetAddress destAddr = fDests->fGroupEId.groupAddress();
  ...
  fDests->fGroupEId = GroupEId(destAddr, destPortNum, Scope(destTTL));
}
...
void Groupsock::addDestination(NetAddress const& addr, Port const& port) {
  ...
  for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
    if (addr == ((struct in_addr)dests->fGroupEId.groupAddress())
...
void Groupsock::removeDestination(NetAddress const& addr, Port const& port) {
  for (destRecord** destsPtr = &fDests; *destsPtr != NULL;
       destsPtr = &((*destsPtr)->fNext)) {
    if (addr == ((struct in_addr)(*destsPtr)->fGroupEId.groupAddress())
...
#if 0
  socketLeaveGroup(env(), socketNum(), fIncomingGroupEId.groupAddress());
  for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
    socketLeaveGroup(env(), socketNum(), dests->fGroupEId.groupAddress());
  }
#endif
}
...
Boolean Groupsock::output(UsageEnvironment& env, u_int8_t ttlToSend,
  ...
  do {
    ...
    for (destRecord* dests = fDests; dests != NULL; dests = dests->fNext) {
      if (!write(dests->fGroupEId.groupAddress(), dests->fPort, ttlToSend,
		 buffer, bufferSize)) {
	writeSuccess = False;
	break;
      }
...
Boolean Groupsock::handleRead(unsigned char* buffer, unsigned bufferMaxSize,
  ...
  if (isSSM()
      && fromAddress.sin_addr.s_addr != ((struct in_addr)sourceFilterAddress()).s_addr) {
    return True;
  }
...
int Groupsock::outputToAllMembersExcept(DirectedNetInterface* exceptInterface,
  ...
  while ((interf = iter.next()) != NULL) {
	...
	trailer = trailerInPacket;
      ...
      if (fDests != NULL) {
	trailer->address() = ((struct in_addr)fDests->fGroupEId.groupAddress()).s_addr;
	trailer->port() = fDests->fPort; // structure copy, outputs in network order
      ...
      if (isSSM()) {
	trailer->auxAddress() = ((struct in_addr)sourceFilterAddress()).s_addr;
      }
...
Boolean GroupsockLookupTable::Remove(Groupsock const* groupsock) {
  ...
  return fTable.Remove(((struct in_addr)groupsock->groupAddress()).s_addr,
		       ((struct in_addr)groupsock->sourceFilterAddress()).s_addr,
		       groupsock->port());
}
