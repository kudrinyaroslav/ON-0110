...
int setupDatagramSocket(UsageEnvironment& env, Port port, int af) {
  if (!initializeWinsockIfNecessary()) {
    socketErr(env, "Failed to initialize 'winsock': ");
    return -1;
  }

  int newSocket = createSocket(SOCK_DGRAM, af);
  if (newSocket < 0) {
...
#if 1
    NetAddress adr(af, 0);
    Endpoint ep(adr, port);
    if (bind(newSocket, ep.adr(), ep.size()) != 0) {
#else
    MAKE_SOCKADDR_IN(name, addr, port.num());
    if ((af == AF_INET) && bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
#endif
      char tmpBuffer[100];
...
int setupStreamSocket(UsageEnvironment& env,
                      Port port, Boolean makeNonBlocking, int af) {
  if (!initializeWinsockIfNecessary()) {
    socketErr(env, "Failed to initialize 'winsock': ");
    return -1;
  }

  int newSocket = createSocket(SOCK_STREAM, af);
  if (newSocket < 0) {
...
int readSocket(UsageEnvironment& env,
    ...
    int bytesRead = recvfrom(socket, (char*)buffer, bufferSize, 0,
      ...
      &addressSize);
    if (_fromAddress.sin6_family == AF_INET) {
      memcpy(&fromAddress, &_fromAddress, sizeof(fromAddress));
    }
#else
...
Boolean writeSocket(UsageEnvironment& env,
		    int socket, NetAddress const& address, Port port,
		    u_int8_t ttlArg,
...
Boolean socketJoinGroup(UsageEnvironment& env, int socket, NetAddress const& groupAddress, u_int32_t nicIndex) {
  ...
  } else if (groupAddress.af() == AF_INET) {
    struct ip_mreq imr;
