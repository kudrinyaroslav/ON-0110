...
Socket::Socket(UsageEnvironment& env, Port port, int af)
  : fEnv(DefaultUsageEnvironment != NULL ? *DefaultUsageEnvironment : env), fPort(port), fAF(af) {
  fSocketNum = setupDatagramSocket(fEnv, port, af);
}
...
Boolean Socket::changePort(Port newPort) {
  ...
  fSocketNum = setupDatagramSocket(fEnv, newPort, fAF);
  if (fSocketNum < 0) {
