...
public:
  OutputSocket(UsageEnvironment& env, int af = AF_INET);
  virtual ~OutputSocket();

  Boolean write(NetAddress const& address, Port port, u_int8_t ttl,
		unsigned char* buffer, unsigned bufferSize);

protected:
  OutputSocket(UsageEnvironment& env, Port port, int af = AF_INET);
...
public:
  Groupsock(UsageEnvironment& env, NetAddress const& groupAddr,
  ...
  void changeDestinationParameters(NetAddress const& newDestAddr,
  ...
  void removeDestination(NetAddress const& addr, Port const& port);
  void removeAllDestinations();

  NetAddress const& groupAddress() const {
    return fIncomingGroupEId.groupAddress();
  }
  NetAddress const& sourceFilterAddress() const {
