...
static void CreateCNonce(char* CNonceVal) {
...
Authenticator::Authenticator(char const* username, char const* password, Boolean passwordIsMD5) {
  assign(NULL, NULL, username, password, passwordIsMD5, False);
  CreateCNonce(fCNonceVal);
}

Authenticator::Authenticator(const Authenticator& orig) {
  assign(orig.realm(), orig.nonce(), orig.username(), orig.password(), orig.fPasswordIsMD5, orig.fQOP);
  CreateCNonce(fCNonceVal);
}
