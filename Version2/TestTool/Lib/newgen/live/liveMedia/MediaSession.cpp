...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
  ...
  while (sdpLine != NULL) {
    ...
    while (1) {
      ...
      // AS update by RFC6064
      if (subsession->parseSDPAttribute_framesize(sdpLine)) continue;
      // AS update by RFC6064 end
      // AS update for backchannel
      if (subsession->parseSDPAttribute_backflag(sdpLine)) continue;
      // AS update for backchannel end
...
static char* parseCLine(char const* sdpLine) {
  ...
  // AS update to fix for correct line end parsing and avoiding wrong DNS lookup
  // AS update to allow IPv6 address
  char IPvNum = '\0';
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
	...
	if (fMultiplexRTCPWithRTP) {
      ...
      // AS update for c=IN IP4 0.0.0.0 issue with IPv6 connection
      // if connection endpoint address is zero, alter it with control address
      if (!tempAddr.isNotZero()) {
        char* username;
        char* password;
        NetAddress destAddress;
        portNumBits urlPortNum;
        char const* urlSuffix;
        const char* Path = fControlPath;
        if (!Path) {
          Path = parentSession().controlPath();
        }
        if (Path) {
          if (RTSPClient::parseRTSPURL(parentSession().envir(), Path, username, password, destAddress, urlPortNum, &urlSuffix)) {
            if (tempAddr.af() != destAddress.af()) {
              fWarningMessage = "Different IP types in connection and connection description";
              tempAddr = destAddress;
            }
          }
        }
      }
      // AS update for c=IN IP4 0.0.0.0 issue with IPv6 connection end
...
NetAddress MediaSubsession::connectionEndpointAddress() const {
  do {
...
void MediaSubsession::setDestinations(NetAddress const& defaultDestAddress) {
...
// AS update for backchannel
Boolean MediaSubsession::parseSDPAttribute_backflag(char const* sdpLine) {
    // Check for a "a=sendonly|recvonly"
    Boolean parseSuccess = False;
    
    if (strncmp(sdpLine, "a=sendonly", 10) == 0) {
        parseSuccess = True;
        fBackchannel = True;
    } else if (strncmp(sdpLine, "a=recvonly", 10) == 0) {
        parseSuccess = True;
        fBackchannel = False;
    }
