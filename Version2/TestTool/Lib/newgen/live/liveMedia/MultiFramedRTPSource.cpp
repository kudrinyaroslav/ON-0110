...
// AS update for ONVIF extensions
Boolean MultiFramedRTPSource::processExtData(BufferedPacket* packet, unsigned extHdr) 
{ 
  return False; 
}
// AS update for ONVIF extensions end
...
void MultiFramedRTPSource::doGetNextFrame() {
  ...
  fNeedDelivery = True;
  int SpecialheaderStatus;
  doGetNextFrame1(SpecialheaderStatus);
}

void MultiFramedRTPSource::doGetNextFrame1(int &SpecialheaderStatus) {
  while (fNeedDelivery) {
...
void MultiFramedRTPSource::networkReadHandler1() {
  ...
  do {
    ...
    rtpHdr = ntohl(*(u_int32_t*)(bPacket->data())); ADVANCE(2);
    ...
    if (rtpPayloadFormat() && rtpPayloadType != rtpPayloadFormat()) {
    ...
    if (rtpHdr&0x10000000) {
      ...
      if (!processExtData(bPacket, extHdr)) {
        ADVANCE(remExtSize);
      }
  ...
  if (fRtpExtHdrCallback) {
    if ((extHdr >> 16) == 0xFFD8 || (extHdr >> 16) == 0xFFFF) {
      extensionData[0] = SpecialHeaderStatus;
      extensionData[1] = fCurrentPacketBeginsFrame ? 1 : 0;
      fRtpExtHdrCallback(
        (extHdr >> 16), seqNumber, sizeof(extensionData), 
        (u_int8_t*)extensionData, fRtpExtHdrCallbackPrivData); 
    }

    // AS update for replay extension header end
    if (readExtension) {
      extensionData[2] += ((fCurrentPacketBeginsFrame              ? 1 << 1 : 0) << 16) 
                        + ((fCurrentPacketCompletesFrame           ? 1 << 2 : 0) << 16) 
                        + ((fCurPacketHasBeenSynchronizedUsingRTCP ? 1 << 3 : 0) << 16)
                        + ((fCurPacketMarkerBit                    ? 1 << 4 : 0) << 16);
      extensionData[3] = fCurPacketRTPTimestamp;
      // payload type
      extensionData[4] = (unsigned char)((rtpHdr & 0x007F0000) >> 16);
      fRtpExtHdrCallback(
        0xABAC, seqNumber, 
        sizeof(extensionData), (u_int8_t*)extensionData, fRtpExtHdrCallbackPrivData); 
    }
    // AS update for replay extension header end
  }
}
