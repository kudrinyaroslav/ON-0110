...
unsigned RTSPClient::sendSetParameterCommand(MediaSession& session, responseHandler* responseHandler,
                                             ...
                                             Authenticator* authenticator) {
  if (fCurrentAuthenticator < authenticator) fCurrentAuthenticator = *authenticator;
  // AS update to support simple Keep Alive
  if ((!parameterName || !*parameterName) && (!parameterValue || !*parameterValue)) {
    return sendRequest(new RequestRecord(++fCSeq, "SET_PARAMETER", responseHandler, &session, NULL, False, 0.0, 0.0, 0.0));
  } else {
  ...
  // AS update to support simple Keep Alive end
}
...
void RTSPClient::setCustomFields(char const* customFields)
{
  if (customFields)
  {
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
  ...
  : Medium(env),
    // AS update to start CSeq from 1
    fVerbosityLevel(verbosityLevel), fCSeq(0), fAllowBasicAuthentication(True), fServerAddress(0),
    ...
    fLastSessionId(NULL), fSessionTimeoutParameter(0), fSessionCookieCounter(0), fHTTPTunnelingConnectionIsPending(False), fCustomFields("") {
  setBaseURL(rtspURL);
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
    ...
    // AS update for RTSP message callbacks
    ReflectSend(cmd);
    // AS update for RTSP message callbacks end
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0) {
    ...
    if (streamUsingTCP) { // streaming over the RTSP connection
      ...
      Boolean requestMulticastStreaming
        = IsMulticastAddress(connectionAddress) || (!connectionAddress.isNotZero() && forceMulticastOnUnspecified);
      transportTypeStr = requestMulticastStreaming ? ";multicast" : ";unicast";
// AS update basing on http://www.ietf.org/rfc/rfc2326.txt - page 59
...
int RTSPClient::openConnection() {
  do {
    ...
    fInputSocketNum = fOutputSocketNum = setupStreamSocket(envir(), 0, True, destAddress.af());
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
  /*
  MAKE_SOCKADDR_IN(remoteName, ((struct in_addr)fServerAddress).s_addr, htons(remotePortNum));
  ...
  if (fServerAddress.connect(socketNum, htons(remotePortNum)) != 0){
  */
  Endpoint ep(fServerAddress, Port(remotePortNum));
  if (connect(socketNum, ep.adr(), ep.size()) != 0) {
    ...
    if (auth.nonce() != NULL) { // Digest authentication
      ...
      if (auth.qop()) {
        ...
        unsigned authBufSize = strlen(authFmt)
          ...
          + strlen(auth.nonce()) + strlen(url) + strlen(auth.cnonce()) + strlen(response);
        authenticatorStr = new char[authBufSize];
        sprintf(authenticatorStr, authFmt,
          auth.username(), auth.realm(),
          auth.nonce(), url, auth.cnonce(), response);
      ...
      // AS update to support QOP mode end
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession& subsession, char const* sessionParamsStr, char const* transportParamsStr,
  ...
  do {
	...
	subsession.rtpSource()->enableRTCPReports() = False;
      ...
      NetAddress destAddress = subsession.connectionEndpointAddress();
      if (!destAddress.isNotZero()) {
...
Boolean RTSPClient::handleAuthenticationFailure(char const* paramsStr) {
  ...
  // AS update to support QOP mode
  char* alg = strDupSize(paramsStr);
  char* qop = strDupSize(paramsStr);
  strcpy(alg, "md5");
  *qop = 0;
  Boolean success = True;
  if (sscanf(paramsStr, "Digest realm=\"%[^\"]\", nonce=\"%[^\"]\", algorithm=%[^,\"], qop=\"%[^\"]\"", 
    realm, nonce, alg, qop) >= 2) {
    fCurrentAuthenticator.setRealmAndNonce(realm, nonce);
    if (qop && *qop) {
      fCurrentAuthenticator.setQOP();
    }
  ...
  delete[] realm; delete[] nonce; delete[] alg, delete[] qop;
  // AS update to support QOP mode end
...
void RTSPClient::responseHandlerForHTTP_GET1(int responseCode, char* responseString) {
  ...
  do {
    ...
    fOutputSocketNum = setupStreamSocket(envir(), 0, True, fServerAddress.af());
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  // AS update for RTSP message callbacks
  ReflectReceive(fResponseBuffer);
  // AS update for RTSP message callbacks end
