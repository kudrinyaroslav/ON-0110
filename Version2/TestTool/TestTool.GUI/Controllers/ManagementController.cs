...
using System.Linq;
using System.Xml;
using TestTool.GUI.Utils.ServiceProviders;
...
namespace TestTool.GUI.Controllers
{
    ...
    class ManagementController : Controller<IManagementView>
    {
        ...
        public override void UpdateContext()
        {
            ...
            environment.TestSettings.FirmwareFilePath = View.SettingsView.FirmwareFilePath;
            environment.TestSettings.CredentialIdentifierValueFirst = View.SettingsView.CredentialIdentifierValueFirst;
            environment.TestSettings.CredentialIdentifierValueSecond = View.SettingsView.CredentialIdentifierValueSecond;
            environment.TestSettings.CredentialIdentifierValueThird = View.SettingsView.CredentialIdentifierValueThird;
        ...
        public override void UpdateView()
        {
            ...
            View.SettingsView.FirmwareFilePath = environment.TestSettings.FirmwareFilePath;

            View.SettingsView.CredentialIdentifierValueFirst = environment.TestSettings.CredentialIdentifierValueFirst;
            View.SettingsView.CredentialIdentifierValueSecond = environment.TestSettings.CredentialIdentifierValueSecond;
            View.SettingsView.CredentialIdentifierValueThird = environment.TestSettings.CredentialIdentifierValueThird;
        ...
        public Profile SaveCurrentProfile(string profileName)
        {
            ...
            profile.FirmwareFilePath = View.SettingsView.FirmwareFilePath;
            profile.CredentialIdentifierValueFirst = View.SettingsView.CredentialIdentifierValueFirst;
            profile.CredentialIdentifierValueSecond = View.SettingsView.CredentialIdentifierValueSecond;
            profile.CredentialIdentifierValueThird = View.SettingsView.CredentialIdentifierValueThird;
...

            {
...

                {
                    ...
                    if (env.TestSettings != null)
                    {
                        ...
                        parameters.TestParameters.FirmwareFilePath = env.TestSettings.FirmwareFilePath;
                        parameters.TestParameters.CredentialIdentifierValueFirst = env.TestSettings.CredentialIdentifierValueFirst;
                        //parameters.TestParameters.CredentialIdentifierValueSecond = env.TestSettings.CredentialIdentifierValueSecond;
            ...
            try
            {
                ...
                {                        
                    ...
                    { 
                        ...
                        devEnv.TestSettings.FirmwareFilePath = testParameters.FirmwareFilePath;
                        devEnv.TestSettings.CredentialIdentifierValueFirst = testParameters.CredentialIdentifierValueFirst;
                        //devEnv.TestSettings.CredentialIdentifierValueSecond = testParameters.CredentialIdentifierValueSecond;
        ...
        public override void LoadSavedContext(SavedContext context)
        {
            if(context.DeviceEnvironment != null)
            {
                ...
                if(context.DeviceEnvironment.TestSettings != null)
                {
                    ...
                    View.SettingsView.FirmwareFilePath = context.DeviceEnvironment.TestSettings.FirmwareFilePath;
                    View.SettingsView.CredentialIdentifierValueFirst = context.DeviceEnvironment.TestSettings.CredentialIdentifierValueFirst;
                    View.SettingsView.CredentialIdentifierValueSecond = context.DeviceEnvironment.TestSettings.CredentialIdentifierValueSecond;
                    View.SettingsView.CredentialIdentifierValueThird = context.DeviceEnvironment.TestSettings.CredentialIdentifierValueThird;
        ...
        public void QueryCredentialIdentifierTypes()
        {
            DiscoveredDevices devices = ContextController.GetDiscoveredDevices();
            string address = devices != null ? devices.ServiceAddress : string.Empty;
            DeviceEnvironment env = ContextController.GetDeviceEnvironment();
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    string credentialServiceAddress = string.Empty;
                    ...
                    //        if (capabilities.Extension.Recording != null)
                    //        {
                    //            credentialServiceAddress = capabilities.Extension.Recording.XAddr;
                    //        }
                    //    }
                    //}

                    if (string.IsNullOrEmpty(credentialServiceAddress))
                    {
                        if (info.Services != null)
                        {
                            var service = Tests.Common.CommonUtils.Extensions.FindService(info.Services, OnvifService.CREDENTIAL_SERVICE);
                            if (service != null)
                            {
                                credentialServiceAddress = service.XAddr;
                            }
                        }
                        else
                        {
                    ...
                    if (string.IsNullOrEmpty(credentialServiceAddress))
                    {
                        throw new Exception("Device does not support Credential service");
                    }

                    var client = new CredentialServiceProvider(credentialServiceAddress, env.Timeouts.Message);
                    client.Security = deviceClient.Security;

                    var supportedTypes = client.GetSupportedCredentialIdentifierTypes();
                    
                    View.SettingsView.SetSupportedCredentialIdentifierTypes(supportedTypes);

                    if (!supportedTypes.Any())
                        throw new Exception("No CredentialIdentifier Types returned by device");
                }
                catch (System.Exception ex)
                {
                    View.ShowError(ex);
        ...
        public void QueryCredentialIdentifierFormatTypes(string typeName)
        {
            DiscoveredDevices devices = ContextController.GetDiscoveredDevices();
            string address = devices != null ? devices.ServiceAddress : string.Empty;
            DeviceEnvironment env = ContextController.GetDeviceEnvironment();
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    {
                        View.SettingsView.SetSupportedCredentialIdentifierFormatTypes(typeName, new string[] {});
                        return;
                    }
                    ...
                    var supportedFormatTypes = client.GetSupportedCredentialIdentifierFormatTypes(typeName);
                    
                    View.SettingsView.SetSupportedCredentialIdentifierFormatTypes(typeName, supportedFormatTypes);

                    if (!supportedFormatTypes.Any())
                        throw new Exception("No CredentialIdentifier Format Types returned by device");
                }
                catch (System.Exception ex)
                {
                    View.ShowError(ex);
