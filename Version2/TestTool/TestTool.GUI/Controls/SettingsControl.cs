...
namespace TestTool.GUI.Controls
{
    ...
    //    }
    //}
    //partial class SettingsControl : Proxy, ITestSettingsView
    //#else
    ...
    /// </summary>
    {
        ...
        {
            get { return tbFirmawareFilePath.Text ?? string.Empty; }
            set { tbFirmawareFilePath.Text = value ?? string.Empty; }
        }

        private void SyncCredentialIdentifierValuesToGUI()
        {
            this.InitiatedByUser = false;

            Action<TextBox, CredentialIdentifierValue> setValue =
                (valueTextBox, value) => valueTextBox.Text = (null != value && null != value.Value) ? Encoding.UTF8.GetString(value.Value) : string.Empty;

            Action<ComboBox, CredentialIdentifierValue> setCredentialIdentifierFormatType =
                        (comboBox, value) => SetSupportedCredentialIdentifierFormatType(comboBox, null != value ? value.FormatType : string.Empty);

            var typeNameFirst = string.Empty;
            if (null != m_CredentialIdentifierValueFirst && !string.IsNullOrEmpty(m_CredentialIdentifierValueFirst.TypeName))
                typeNameFirst = m_CredentialIdentifierValueFirst.TypeName;

            if (null != m_CredentialIdentifierValueSecond && !string.IsNullOrEmpty(m_CredentialIdentifierValueSecond.TypeName))
                typeNameFirst = m_CredentialIdentifierValueSecond.TypeName;

            SetSupportedCredentialIdentifierType(m_CredentialIdentifierTypeFirstComboBox, new[] { typeNameFirst });

            var typeNameSecond = string.Empty;
            if (null != m_CredentialIdentifierValueThird && !string.IsNullOrEmpty(m_CredentialIdentifierValueThird.TypeName))
                typeNameSecond = m_CredentialIdentifierValueThird.TypeName;

            SetSupportedCredentialIdentifierType(m_CredentialIdentifierTypeSecondComboBox, new[] { typeNameSecond });


            setCredentialIdentifierFormatType(m_CredentialIdentifierFormatTypeFirstComboBox, m_CredentialIdentifierValueFirst);
            setCredentialIdentifierFormatType(m_CredentialIdentifierFormatTypeSecondComboBox, m_CredentialIdentifierValueSecond);
            setCredentialIdentifierFormatType(m_CredentialIdentifierFormatTypeThirdComboBox, m_CredentialIdentifierValueThird);

            setValue(m_CredentialIdentifierValueFirstTextBox, m_CredentialIdentifierValueFirst);
            setValue(m_CredentialIdentifierValueSecondTextBox, m_CredentialIdentifierValueSecond);
            setValue(m_CredentialIdentifierValueThirdTextBox, m_CredentialIdentifierValueThird);

            this.InitiatedByUser = true;
        }


        private CredentialIdentifierValue m_CredentialIdentifierValueFirst;
        public CredentialIdentifierValue CredentialIdentifierValueFirst
        {
            get
            {
                if (!string.IsNullOrEmpty(m_CredentialIdentifierTypeFirstComboBox.Text) 
                 && !string.IsNullOrEmpty(m_CredentialIdentifierFormatTypeFirstComboBox.Text))
                    return new CredentialIdentifierValue(m_CredentialIdentifierTypeFirstComboBox.Text, 
                                                         m_CredentialIdentifierFormatTypeFirstComboBox.Text, 
                                                         Encoding.UTF8.GetBytes(m_CredentialIdentifierValueFirstTextBox.Text));

                return null;
            }
            set
            {
                m_CredentialIdentifierValueFirst = value;
                SyncCredentialIdentifierValuesToGUI();
            }
        }

        private CredentialIdentifierValue m_CredentialIdentifierValueSecond;
        public CredentialIdentifierValue CredentialIdentifierValueSecond
        {
            get
            {
                if (!string.IsNullOrEmpty(m_CredentialIdentifierTypeFirstComboBox.Text) 
                 && !string.IsNullOrEmpty(m_CredentialIdentifierFormatTypeSecondComboBox.Text))
                    return new CredentialIdentifierValue(m_CredentialIdentifierTypeFirstComboBox.Text, 
                                                         m_CredentialIdentifierFormatTypeSecondComboBox.Text, 
                                                         Encoding.UTF8.GetBytes(m_CredentialIdentifierValueSecondTextBox.Text));

                return null;
            }
            set
            {
                m_CredentialIdentifierValueSecond = value;
                SyncCredentialIdentifierValuesToGUI();
            }
        }

        private CredentialIdentifierValue m_CredentialIdentifierValueThird;
        public CredentialIdentifierValue CredentialIdentifierValueThird
        {
            get
            {
                if (!string.IsNullOrEmpty(m_CredentialIdentifierTypeSecondComboBox.Text) 
                 && !string.IsNullOrEmpty(m_CredentialIdentifierFormatTypeThirdComboBox.Text))
                    return new CredentialIdentifierValue(m_CredentialIdentifierTypeSecondComboBox.Text, 
                                                         m_CredentialIdentifierFormatTypeThirdComboBox.Text, 
                                                         Encoding.UTF8.GetBytes(m_CredentialIdentifierValueThirdTextBox.Text));

                return null;
            }
            set
            {
                m_CredentialIdentifierValueThird = value;
                SyncCredentialIdentifierValuesToGUI();
            }
        }
        /// <summary>
        ...
        public int OperationDelay
        {
            ...
            set
            {
                _lastValidOperationDelay = Math.Max(value, MinimalOperationDelay);
        ...
        {
            get { return tbRetentionTime.Text; }
            set { tbRetentionTime.Text = value; }
        }
        ...
        public const int MinimalOperationDelay = 1000;
        private int _lastValidOperationDelay = MinimalOperationDelay;
        ...
        private void tbOperationDelay_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (bOk)
            {
                bOk = time >= MinimalOperationDelay;
            }
            if (!bOk)
            {
                tbOperationDelay.Text = _lastValidOperationDelay.ToString();
        ...
        public override Controllers.IController GetController()
        {
            return null;
        }
        ...
        public void ShowMissingSettings(string name)
        {
            switch (name)
            {
                case Controllers.ManagementController.EVENTSSETTINGS:
                    {
                        tcMiscSettings.SelectedTab = tpEvents;
                        if (string.IsNullOrEmpty(cmbEventTopic.Text))
                        {
                            cmbEventTopic.Focus();
                        }
                        else
                        {
                            if (string.IsNullOrEmpty(tbNamespaces.Text))
                            {
                                tbNamespaces.Focus();
                            }
                        }
        ...
        public void SetRecordings(IEnumerable<string> recordings)
        {
            Invoke(new Action(
                () =>
                {
                    foreach (ComboBox cb in new ComboBox[]{cmbRecordingToken})
                    {
                        ...
                        {
                            cb.Items.Add(recording);
                        }
                        if (cb.Items.Count > 0)
                        {
                            if (cb.Items.Contains(backup))
                            {
                                cb.SelectedItem = backup;
                            }
                            else
                            {
                                cb.SelectedIndex = 0;
        ...
        #region ITestSettingsView Members
        ...
        public string MetadataFilter
        {
            get
            {
                return tbMetadataFilter.Text;
            }
            set
            {
                tbMetadataFilter.Text = value;
            }
        }
        ...
        {
            if (_isTextChangedHandlerRunning)
                return;

            _isTextChangedHandlerRunning = true;
            int saveStart = tbMetadataFilter.SelectionStart;
            int saveLength = tbMetadataFilter.SelectionLength;

            tbMetadataFilter.SelectAll();
            tbMetadataFilter.SelectionBackColor = System.Drawing.SystemColors.Window;

            tbMetadataFilter.SelectionStart = saveStart;
            tbMetadataFilter.SelectionLength = saveLength;
            _isTextChangedHandlerRunning = false;
        }

        public event EventHandler QuerySupportedCredentialIdentifierTypesRequested;

        public void SetSupportedCredentialIdentifierType(ComboBox comboBox, IEnumerable<string> types)
        {
            Invoke(new Action(() =>
                              {
        ...
        public void SetSupportedCredentialIdentifierTypes(IEnumerable<string> types)
        {
            SetSupportedCredentialIdentifierType(m_CredentialIdentifierTypeFirstComboBox, types);
            SetSupportedCredentialIdentifierType(m_CredentialIdentifierTypeSecondComboBox, types);
        }

        private void m_CredentialIdentifierTypeGetButton_Click(object sender, EventArgs e)
        {
            if (null != QuerySupportedCredentialIdentifierTypesRequested)
                QuerySupportedCredentialIdentifierTypesRequested(sender, e);
        }

        public event EventHandler QuerySupportedCredentialIdentifierFormatTypesRequested;
        public void SetSupportedCredentialIdentifierFormatType(ComboBox comboBox, string type)
        {
            SetSupportedCredentialIdentifierFormatType(comboBox, (string.IsNullOrEmpty(type) ? new string[0] : new [] { type }));
        }
        public void SetSupportedCredentialIdentifierFormatType(ComboBox comboBox, IEnumerable<string> types)
        {
            Invoke(new Action(() =>
                              {
                                  comboBox.SelectedIndex = -1;
        ...
        public void SetSupportedCredentialIdentifierFormatTypes(string typeName, IEnumerable<string> types)
        {
            Invoke(new Action(() =>
                              {
                                  if (m_CredentialIdentifierTypeFirstComboBox.Text == typeName)
                                  {
                                      SetSupportedCredentialIdentifierFormatType(m_CredentialIdentifierFormatTypeFirstComboBox, types);
                                      SetSupportedCredentialIdentifierFormatType(m_CredentialIdentifierFormatTypeSecondComboBox, types);
                                  }

                                  if (m_CredentialIdentifierTypeSecondComboBox.Text == typeName)
                                      SetSupportedCredentialIdentifierFormatType(m_CredentialIdentifierFormatTypeThirdComboBox, types);
                              }));
        }

        private void m_CredentialIdentifierTypeComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            OnCredentialIdentifierValuesChanged(sender, e);
        }

        private void SyncCredentialIdentifierValues(object sender, EventArgs eventArgs)
        {
            if (InitiatedByUser && null != QuerySupportedCredentialIdentifierFormatTypesRequested)
            {
                QuerySupportedCredentialIdentifierFormatTypesRequested(sender, eventArgs);
            }
        }
    }
}
