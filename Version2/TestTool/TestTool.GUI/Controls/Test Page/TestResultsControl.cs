...
namespace TestTool.GUI.Controls
{
    public partial class TestResultsControl : UserControl, ITestResultView
    {
        ...
        // mark warning by blue color
        private void HighlightWarnings(string PlainLog)
        {
            int posWarning = -1;
            int startPos = 0;

            while ((posWarning = rtbTestResult.Find(warningStr, posWarning + 1, RichTextBoxFinds.None)) >= 0)
            {
                int warningTotalLength = 0;

                // get line number of warning string
                int lineNumber = rtbTestResult.GetLineFromCharIndex(posWarning);
                warningTotalLength += (rtbTestResult.Lines[lineNumber].Length + 1);

                // get first char index of found line
                startPos = rtbTestResult.GetFirstCharIndexFromLine(lineNumber);

                rtbTestResult.Select(startPos, warningTotalLength);
                rtbTestResult.SelectionColor = System.Drawing.Color.Blue;
        ...
        public void DisplayTestResults(TestResult log)
        {
            ...
            if (rtbTestResult.TextLength != 0)
            {
                ...
                // highlight warnings in new log with blue color
                HighlightWarnings(rtbTestResult.Text);
            }
        }
        ...
        public void ClearTestInfo()
        {
            ...
            // reset "warnings" for new test log
            lstWarningPos.Clear();
            lstWarningLength.Clear();
        }
        ...
        // list that store warning message positions
        private System.Collections.Generic.List<int> lstWarningPos = new System.Collections.Generic.List<int>();

        // list that store length of warning message
        private System.Collections.Generic.List<int> lstWarningLength = new System.Collections.Generic.List<int>();

        private int FailedStepCurrentFoundPos = -1;
        private int WarningCurrentFoundPos = -1;
        ...
        void InternalWriteLine(string logEntry)
        {
            ...
            // mark warning by blue color
            if (logEntry.ToLower().Contains(warningStr))
            {
                int posWarning = -1;
                posWarning = rtbTestResult.Find(warningStr, WarningCurrentFoundPos + 1, RichTextBoxFinds.None);
                WarningCurrentFoundPos = posWarning;
                ...
                if (posWarning >= 0)
                {
                    ...
                    // remember start position of warning message
                    lstWarningPos.Add(startPos);
                    // remember length of warning message
                    lstWarningLength.Add(warningTotalLength);
            ...
            if (logEntry == stepFailedStr)
            {
                ...
                if (posFailed >= 0)
                {
                    ...
                    // if failed step contains warning messages then mark warnings one more time
                    // because they were marked with red color already
                    for (int i = 0; i < lstWarningPos.Count; i++)
                    {
                        if (lstWarningPos[i] > startPos &&
                             lstWarningPos[i] + lstWarningLength[i] < startPos + stepFailedTotalLength)
                        {
                            rtbTestResult.Select(lstWarningPos[i], lstWarningLength[i]);
                            rtbTestResult.SelectionColor = System.Drawing.Color.Blue;
                        }
                    }
                }
