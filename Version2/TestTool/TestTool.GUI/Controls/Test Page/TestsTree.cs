...
namespace TestTool.GUI.Controls
{
    public partial class TestsTree : UserControl, ITestTreeView
    {
        ...
        /// </summary>
        private ExecutableTestList _selectedTests;
 
        public ExecutableTestList SelectedTests
        {
        ...
        public void DisplayTests(IEnumerable<TestInfo> tests)
        {
            try
            {
                foreach (TestInfo testInfo in tests.OrderBy(ti => ti.Category).ThenBy(ti => ti.Order))
                {
                    ...
                    // Add node
#if __PROFILE_A__
                    testInfo.Name = string.Format("{0}-{1}-{2} {3}", testInfo.Category, testInfo.Id, testInfo.LastChangedIn, testInfo.Name);
#else
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine(testInfo.Name + " NO FUNCTIONALITY UNDER TEST!");
                    }
#else
        ...
        string CreateTestTooltip(TestInfo testInfo, TestResult testResult)
        {
            ...
            string functionalityUnderTest = testInfo.GetFunctionalityString();

            string tooltip = string.Format("{0}\r\n{1} \r\nFunctionality under test: {2}\r\nState: {3}",
                testName, requirementLevel, functionalityUnderTest, state);
        ...
        public void ApplyProfileOptions(Profile profile)
        {
            ...
            {
                _testNodes[testInfo].Checked = profile.TestCases.Contains(string.Format("{0}|{1}", testInfo.Category, testInfo.Order));
            }
        ...
        public void HighlightCompletedTest(TestResult testResult)
        {
            if (_testNodes.ContainsKey(testResult.TestInfo))
            {
                ...
                node.BackColor = tvTestCases.BackColor;
        ...
        private void tvTestCases_BeforeCheck(object sender, TreeViewCancelEventArgs e)
        {
            ...
            //Supress selecting/deselecting of test under execution
            if (null != e.Node.Tag)
            {
                if (SelectedTests.TestUnderExecution == e.Node.Tag)
                {
                    e.Cancel = true;
                    return;
                }
        ...
        private void tvTestCases_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (AfterSelect != null)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Crash in AfterSelect");
                }
            }
        }
        ...
        private void tvTestCases_BeforeSelect(object sender, TreeViewCancelEventArgs e)
        {
            if (BeforeSelect != null)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Crash in BeforeSelect");
                }
            }
        }
        ...
        {
            //System.Diagnostics.Debug.WriteLine(string.Format("DoubleClick: Clicks: {0}, Delta: {1}", e.Clicks, e.Delta));

        }

        #region Context menu

        private TreeNode _rightClickedNode = null;

        private void cmsTestsTree_Opening(object sender, System.ComponentModel.CancelEventArgs e)
        {
            TreeNode node = _rightClickedNode;
            if (node == null)
            {
                expandCurrentToolStripMenuItem.Enabled = false;
                collapseCurrentToolStripMenuItem.Enabled = false;
            }
            else
            {
                expandCurrentToolStripMenuItem.Enabled = CanBeExpanded(node);
                collapseCurrentToolStripMenuItem.Enabled = CanBeCollapsed(node);
            }

            bool canBeExpanded = false;
            bool canBeCollapsed = false;
            foreach (TreeNode n in tvTestCases.Nodes)
            {
                if (CanBeExpanded(n))
                {
                    canBeExpanded = true;
            ...
            expandAllToolStripMenuItem.Enabled = canBeExpanded;
            collapseAllToolStripMenuItem.Enabled = canBeCollapsed;
        }
        ...
        private void tvTestCases_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                _rightClickedNode = tvTestCases.GetNodeAt(e.X, e.Y);
                if (_rightClickedNode != null)
                {
                    _rightClickedNode.BackColor = Color.LightSkyBlue;
                }
            }
        }
        ...
        private void cmsTestsTree_Closing(object sender, ToolStripDropDownClosingEventArgs e)
        {
            if (_rightClickedNode != null)
            {
                _rightClickedNode.BackColor = tvTestCases.BackColor;
            }
        ...
        bool CanBeExpanded(TreeNode node)
        {
            ...
            else
            {
                foreach (TreeNode child in node.Nodes)
                {
                    bool canBeExpanded = CanBeExpanded(child);
                    if (canBeExpanded)
                    {
                        return true;
                    }
        ...
        bool CanBeCollapsed(TreeNode node)
        {
            if (node.IsExpanded && node.Nodes.Count > 0)
            {
                return true;
