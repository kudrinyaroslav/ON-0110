...
namespace TestTool.Tests.Common.CommonUtils
{
    ...
    public class TopicInfo
    {
        ...
        /// <returns></returns>
        public static TopicInfo ExtractTopicInfoAll(string topicString, XmlNode topicNode)
        {
        ...
        internal static TopicInfo ExtractTopicInfoPACS(string topicString, Func<string, string> resolver)
        {
            TopicInfo currentTopicInfo = null;
            ...
            for (int i = 0; i < topicParts.Length; i++)
            {
                ...
                {
                    topicName = currentTopicParts[1];
                    topicNamespace = resolver(currentTopicParts[0]);
                    namespacePrefix = currentTopicParts[0];
                }
        ...
        public static TopicInfo ExtractTopicInfoPACS(string topicString, XmlNode topicNode)
        {
            return ExtractTopicInfoPACS(topicString, topicNode.GetNamespaceOfPrefix);
        }
        
        public static TopicInfo ExtractTopicInfoPACS(string topicString, IXmlNamespaceResolver resolver)
        {
            return ExtractTopicInfoPACS(topicString, resolver.LookupNamespace);
        }
        ...
        public static TopicInfo ConstructTopicInfoNested(EventsTopicInfo plainInfo)
        {
            Dictionary<string, string> namespaces = new Dictionary<string, string>();
            string[] definitions = plainInfo.NamespacesDefinition.Replace('"' + Environment.NewLine, "\" ").Split(' ');
            foreach (string definition in definitions)
