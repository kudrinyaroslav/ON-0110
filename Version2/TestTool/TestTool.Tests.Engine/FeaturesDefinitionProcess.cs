...
namespace TestTool.Tests.Engine
{
    ...
    {
        private bool m_MetadataRecordingAlreadyDefined = false;
            ...
            RunTest(() =>
                 {
                     ...
                     _eventsServiceAddress = (null != eventsService) ? eventsService.XAddr : (null != capabilities && null != capabilities.Events ? capabilities.Events.XAddr : null);
                     ...
                     if (capabilities != null || serviceCapabilities != null)
                     {
                         ...
                         //[28.03.2014] AKS: don't change the order of call to DefineRecordingCapabilities and DefineSearchCapabilities.
                         //Or modify definition procedure for MetadataRecording: in DefineSearchCapabilities definition of MetadataRecording's support 
                         //should be performed only if it hasn't been done in DefineRecordingCapabilities.
                         ...
                         DefineAccessRulesCapabilities(services, serviceCapabilities);

                         DefineAdvancedSecurityCapabilities(services, serviceCapabilities);

                         DefineCredentialCapabilities(services, serviceCapabilities);

                         DefineScheduleCapabilities(services, serviceCapabilities);
                     }
        ...
        private void DefineProfileQSupport()
        {
            LogAndNotify(Feature.ProfileQSupported,
                         "Profile Q",
                         DeclaredScopes.Contains(ProfileQ.FactoryDefaultScope) || DeclaredScopes.Contains(ProfileQ.OperationalScope));
            LogStepEvent("");
        }
        ...
        void DefineDeviceScopes()
        {
            ...
            DeclaredScopes.AddRange(_scopes);
        ...
        void DefineDiscoveryFeatures(Capabilities capabilities, DeviceServiceCapabilities serviceCapabilities)
        {
            ...
            if (serviceCapabilities != null)
            {
                ...
                if (serviceCapabilities.System != null)
                    byeSupported = serviceCapabilities.System.DiscoveryByeSpecified && serviceCapabilities.System.DiscoveryBye;
                else
        ...
        void DefineDeviceFeatures(Capabilities capabilities, DeviceServiceCapabilities serviceCapabilities)
        {
            ...
            bool systemHttpFirmwareUpgrade = false;
            bool systemHttpSystemBackup = false;
            bool systemHttpSystemLogging = false;
            bool systemHttpSupportInformation = false;
            //bool byeSupported = false;
            ...
            bool defaultAccessPolicy = false;
            bool maximumUsers = false;
            bool maximumUsernameLength = false;
            bool maximumPasswordLength = false;
            bool remoteUserHandling = false;
            ...
            if (serviceCapabilities != null)
            {
                ...
                if (serviceCapabilities.System != null)
                {
                    ...
                    {
                        LogStepEvent("System Logging feature not specified");
                    }

                    if (serviceCapabilities.System.HttpFirmwareUpgradeSpecified)
                    {
                        systemHttpFirmwareUpgrade = serviceCapabilities.System.HttpFirmwareUpgrade;
                    }
                    else
                    {
                        LogStepEvent("Http Firmware Upgrade feature not specified");
                    }

                    if (serviceCapabilities.System.HttpSystemBackupSpecified)
                    {
                        systemHttpSystemBackup = serviceCapabilities.System.HttpSystemBackup;
                    }
                    else
                    {
                        LogStepEvent("Http System Backup feature not specified");
                    }

                    if (serviceCapabilities.System.HttpSystemLoggingSpecified)
                    {
                        systemHttpSystemLogging = serviceCapabilities.System.HttpSystemLogging;
                    }
                    else
                    {
                        LogStepEvent("Http System Logging feature not specified");
                    }

                    if (serviceCapabilities.System.HttpSupportInformationSpecified)
                    {
                        systemHttpSupportInformation = serviceCapabilities.System.HttpSupportInformation;
                    }
                    else
                    {
                        LogStepEvent("Http Support Information feature not specified");
                    }
                }
                else
                {
                ...
                if (serviceCapabilities.Security != null)
                {
                    ...
                    if (serviceCapabilities.Security.DefaultAccessPolicySpecified)
                    {
                        defaultAccessPolicy = serviceCapabilities.Security.DefaultAccessPolicy;
                    }
                    else
                    {
                        LogStepEvent("Default Access Policy feature not specified");
                    }
                    ...
                    {
                        LogStepEvent("Maximum Users feature not specified");
                    }
                    if (serviceCapabilities.Security.RemoteUserHandlingSpecified)
                    {
                        remoteUserHandling = serviceCapabilities.Security.RemoteUserHandling;
                    }
                    else
                    {
                        LogStepEvent("Remote User Handling feature not specified");
                    }
                    if (serviceCapabilities.Security.MaxUserNameLengthSpecified)
                    {
                        maximumUsernameLength = true;
                    }
                    else
                    {
                        LogStepEvent("Maximum UserName Length feature not specified");
                    }
                    if (serviceCapabilities.Security.MaxPasswordLengthSpecified)
                    {
                        maximumPasswordLength = true;
                    }
                    else
                    {
                        LogStepEvent("Maximum Password Length feature not specified");
                    }
                }
                else
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (capabilities.Device.System != null)
                    {
                        ...
                        //byeSupported = capabilities.Device.System.DiscoveryBye;
                        if (capabilities.Device.System.Extension != null)
                        {
                            if (capabilities.Device.System.Extension.HttpFirmwareUpgradeSpecified)
                            {
                                systemHttpFirmwareUpgrade = capabilities.Device.System.Extension.HttpFirmwareUpgrade;
                            }
                            else
                            {
                                LogStepEvent("HTTP Firmware Upgrade feature not specified");
                            ...
                            if (capabilities.Device.System.Extension.HttpSystemBackupSpecified)
                            {
                                systemHttpSystemBackup = capabilities.Device.System.Extension.HttpSystemBackup;
                            }
                            else
                            {
                                LogStepEvent("HTTP System Backup feature not specified");
                            ...
                            if (capabilities.Device.System.Extension.HttpSystemLoggingSpecified)
                            {
                                systemHttpSystemLogging = capabilities.Device.System.Extension.HttpSystemLogging;
                            }
                            else
                            {
                                LogStepEvent("HTTP System Logging feature not specified");
                            ...
                            if (capabilities.Device.System.Extension.HttpSupportInformationSpecified)
                            {
                                systemHttpSupportInformation = capabilities.Device.System.Extension.HttpSupportInformation;
                            }
                            else
                            {
                                LogStepEvent("HTTP Support Information feature not specified");
                        ...
                        {
                            LogStepEvent("Device.System.Extension field is empty");
                        }
                    }
                    else
                    {
                    ...
                    if (null != capabilities.Device.Security)
                    {
                        if (null != capabilities.Device.Security.Extension && null != capabilities.Device.Security.Extension.Extension)
                            remoteUserHandling = capabilities.Device.Security.Extension.Extension.RemoteUserHandling;
                        else
                            LogStepEvent("Remote User Handling feature not specified");
                    }
                    else
                    {
                        LogStepEvent("Device.Security field is empty");
                    }
            ...
            LogAndNotify(Feature.DefaultAccessPolicy, "Default Access Policy", defaultAccessPolicy);
            LogAndNotify(Feature.MaxUsers, "Maximum Users", maximumUsers);
            LogAndNotify(Feature.RemoteUserHandling, "Remote User Handling", remoteUserHandling);
            LogAndNotify(Feature.MaximumUsernameLength, "Maximum Username Length", maximumUsernameLength);
            LogAndNotify(Feature.MaximumPasswordLength, "Maximum Password Length", maximumPasswordLength);
            StepPassed();
            ...
            BeginStep("Define System features");
            LogAndNotify(Feature.SystemLogging, "System Logging", systemLogging);
            LogAndNotify(Feature.HttpSystemLogging, "HTTP System Logging", systemHttpSystemLogging);
            LogAndNotify(Feature.HttpFirmwareUpgrade, "HTTP Firmware Upgrade", systemHttpFirmwareUpgrade);
            LogAndNotify(Feature.HttpSupportInformation, "HTTP Support Information", systemHttpSupportInformation);
            LogAndNotify(Feature.HttpSystemBackup, "HTTP System Backup", systemHttpSystemBackup);
            StepPassed();
            ...
            if (_ioSupported)
            {
                Action assumeRelayOutputsAndDigitalInputsSupported = new Action(
                        () =>
                        {
                            ...
                            SetUndefined(Feature.RelayOutputs);
        ...
        void DefineImagingFeatures(Capabilities capabilities, Service[] services)
        {
            ...
            if (_imagingSupported)
            {
                bool IrCutSupported = false;
                try
                {
                    VideoSource[] VideoSources = GetVideoSourcesFromMediaOrIO();
                    if (VideoSources != null)
                    {
                        foreach (VideoSource v in VideoSources)
                        {
                            ImagingOptions20 settings = GetImagingOptions(v.token);
                            bool HasOFF = false;
                            bool HasNonOFF = false;
                            if (settings.IrCutFilterModes != null)
                            {
                                foreach (IrCutFilterMode m in settings.IrCutFilterModes)
                                {
                                    if (m == IrCutFilterMode.OFF)
                                    {
                                        HasOFF = true;
                                    }
                                    if (m != IrCutFilterMode.OFF)
                ...
                catch (Exception exc)
                {
                    //LogIfFault(exc);
                    HandleException(exc);
                    LogTestEvent("Warning: unable to detect IrCutfilter from Imaging service" + Environment.NewLine);
                }
                BeginStep("Define IrCutfilter feature");
                LogAndNotify(Feature.IrCutfilterConfiguration, "IrCutfilter Configuration", IrCutSupported);
                StepPassed();
            }
        }
        ...
        void DefineSearchCapabilities(Capabilities capabilities, Service[] services)
        {
            ...
            if (services != null)
            {
                _searchSupported = services.Count(S => S.Namespace != null && S.Namespace.ToLower() == OnvifService.SEARCH) > 0;
                if (_searchSupported)
                {
            ...
            Action assumeSupported = new Action(() =>
                                                {
                                                    if (!m_MetadataRecordingAlreadyDefined)
                                                    {
            ...
            if (_searchSupported)
            {
                ...
                // Get Recording information and check Metadata Recording and PTZ Search Support
                {
                    try
                    {
                        ...
                        {
                            //[28.03.2014] AKS: check if we have defined support of MetadataRecording feature during analyzing of RecordingServiceCapabilities
                            if (!m_MetadataRecordingAlreadyDefined)
                            {
                                bool supported = false;
                                if (recordingInfo.Track != null)
                                {
                                    TrackInformation trackInformation = recordingInfo.Track.FirstOrDefault(T => T.TrackType == TrackType.Metadata);

                                    supported = trackInformation != null && (trackInformation.DataFrom < trackInformation.DataTo);
                                }
                                m_MetadataRecordingAlreadyDefined = true;
        ...
        void DefineRecordingCapabilities(Capabilities capabilities, Service[] services)
        {
            ...
            if (services != null)
            {
                _recordingControlSupported =
                    services.Count(S => S.Namespace != null && S.Namespace.ToLower() == OnvifService.RECORIDING) > 0;
            ...
            if (_recordingControlSupported)
            {
                if (services != null)
                {
                    try
                    {
                        ...
                        LogAndNotify(Feature.MetadataRecording, "Metadata recording",
                                     recordingCapabilities.MetadataRecordingSpecified &&
                                     recordingCapabilities.MetadataRecording);
                        m_MetadataRecordingAlreadyDefined = true;
            ...
            else
            {
                ...
                LogAndNotify(Feature.AudioRecording, "Audio Recording", false);
                LogAndNotify(Feature.RecordingOptions, "Recording Options", false);
                LogAndNotify(Feature.MetadataRecording, "Metadata recording", false);
                m_MetadataRecordingAlreadyDefined = true;
                LogStepEvent(string.Empty);
            }
        }
        ...
        void DefineReplayCapabilities(Capabilities capabilities, Service[] services)
        {
            ...
            if (services != null)
            {
                _replaySupported = services.Count(S => S.Namespace != null && S.Namespace.ToLower() == OnvifService.REPLAY) > 0;
        ...
        void DefineReceiverCapabilities(Capabilities capabilities, Service[] services)
        {
            ...
            if (services != null)
            {
                _receiverSupported = services.Count(S => S.Namespace != null && S.Namespace.ToLower() == OnvifService.RECEIVER) > 0;
        ...
        void DefineAccessRulesCapabilities(Service[] services, DeviceServiceCapabilities capabilities)
        {
            BeginStep("Define Access Rules support");
            bool _arSupported = false;
            if (services != null)
            {
                _arSupported = services.Any(s => s.Namespace != null && s.Namespace.ToLower() == OnvifService.ACCESS_RULES_SERVICE);

                if (_arSupported)
                {
                    Service arService = services.OrderByDescending(s => s.Version == null ? 0 : s.Version.Major).
                                                 ThenByDescending(s => s.Version == null ? 0 : s.Version.Minor).
                                                 FirstOrDefault(s => s.Namespace.ToLower() == OnvifService.ACCESS_RULES_SERVICE);
                    _accessRulesServiceAddress = arService.XAddr;
                }
            }
            LogAndNotify(Feature.AccessRulesService, "Access Rules", _arSupported);
            StepPassed();

            if (_arSupported)
            {
                AccessRulesServiceCapabilities serviceCapabilities = null;
                try
                {
                    serviceCapabilities = GetAccessRulesServiceCapabilities();
                }
                catch (Exception e)
                {
                    StepFailed(e);
                ...
                {
                    LogStepEvent(string.Format("Get Access Rules service capabilities failed. Unable to determine whether features{0}"
                                               + "    'Multiple Schedules per Access Point',{0}"
                                               + " are supported.", Environment.NewLine));
                    SetUndefined(Feature.MultipleSchedulesAccessPoint);
                    LogStepEvent("");
                    return;
                }

                LogAndNotify(Feature.MultipleSchedulesAccessPoint,
                             "Multiple Schedules per Access Point",
                             serviceCapabilities.MultipleSchedulesPerAccessPointSupported);

                LogStepEvent("");
            }
            else
                TransmitUnsupported(Feature.AccessRulesService);
        }
        ...
        void DefineAdvancedSecurityCapabilities(Service[] services, DeviceServiceCapabilities capabilities)
        {
            ...
            if (services != null)
            {
                _asSupported = services.Any(s => s.Namespace != null && s.Namespace.ToLower() == OnvifService.ADVANCED_SECURITY);
            ...
            if (_asSupported)
            {
                ...
                if (null == serviceCapabilities)
                {
                    // The follwoing temprary commented according ticket #971 ('802.1X configurations'{0}")
                    LogStepEvent(string.Format("Get Advanced Security service capabilities failed. Unable to determine whether features{0}"
                                               ...
                                               + "    'Passphrase Management'{0}"
                                               + "    'PKCS#8 Container Upload'{0}"
                                               + "    'PKCS#12 Container Upload'{0}"
                                               + "    'CRLs'{0}"
                                               + "    'Certification path validation policies'{0}"
                                               //+ "    '802.1X configurations'{0}"
                                               + "    'TLS WWW client auth extended key usage extension'{0}"
                                               + "    'TLS client authentication'{0}"
                                               + " are supported.", Environment.NewLine));
                    ...
                    SetUndefined(Feature.PassphraseManagement);
                    SetUndefined(Feature.CRLs);
                    SetUndefined(Feature.CertificationPathValidationPolicies);
                    SetUndefined(Feature.PKCS8RSAKeyPairUpload);
                    SetUndefined(Feature.PKCS12CertificateWithRSAPrivateKeyUpload);
                    SetUndefined(Feature.CRLs);
                    SetUndefined(Feature.CertificationPathValidationPolicies);
                    SetUndefined(Feature.TLSWebClientAuthExtKeyUsage);
                    // Temprary commented according ticket #971
                    //SetUndefined(Feature.TLSClientAuthentication);
                    //SetUndefined(Feature.Configurations802Dot1X);
                    LogStepEvent("");
                    return;
                }
                ...
                LogAndNotify(Feature.PassphraseManagement,
                             "Passphrase Management",
                             null != serviceCapabilities.KeystoreCapabilities && !string.IsNullOrEmpty(serviceCapabilities.KeystoreCapabilities.MaximumNumberOfPassphrases) && uint.Parse(serviceCapabilities.KeystoreCapabilities.MaximumNumberOfPassphrases) > 0);
                LogAndNotify(Feature.CRLs,
                             "CRLs",
                             null != serviceCapabilities.KeystoreCapabilities && !string.IsNullOrEmpty(serviceCapabilities.KeystoreCapabilities.MaximumNumberOfCRLs) && uint.Parse(serviceCapabilities.KeystoreCapabilities.MaximumNumberOfCRLs) > 0);
                LogAndNotify(Feature.CertificationPathValidationPolicies,
                             "Certification path validation policies",
                             null != serviceCapabilities.KeystoreCapabilities && !string.IsNullOrEmpty(serviceCapabilities.KeystoreCapabilities.MaximumNumberOfCertificationPathValidationPolicies) && uint.Parse(serviceCapabilities.KeystoreCapabilities.MaximumNumberOfCertificationPathValidationPolicies) > 0);
                LogAndNotify(Feature.TLSWebClientAuthExtKeyUsage,
                             "TLS WWW client auth extended key usage extension",
                             null != serviceCapabilities.KeystoreCapabilities && serviceCapabilities.KeystoreCapabilities.EnforceTLSWebClientAuthExtKeyUsageSpecified && serviceCapabilities.KeystoreCapabilities.EnforceTLSWebClientAuthExtKeyUsage);
                LogAndNotify(Feature.TLSClientAuthentication,
                             "TLS client authentication",
                             null != serviceCapabilities.TLSServerCapabilities && serviceCapabilities.TLSServerCapabilities.TLSClientAuthSupportedSpecified && serviceCapabilities.TLSServerCapabilities.TLSClientAuthSupported);
                // Temprary commented according ticket #971
                //LogAndNotify(Feature.Configurations802Dot1X,
                //             "802.1X configurations",
                //             null != serviceCapabilities.Dot1XCapabilities && !string.IsNullOrEmpty(serviceCapabilities.Dot1XCapabilities.MaximumNumberOfDot1XConfigurations) && uint.Parse(serviceCapabilities.Dot1XCapabilities.MaximumNumberOfDot1XConfigurations) > 0);
                LogAndNotify(Feature.PKCS8RSAKeyPairUpload,
                             "PKCS#8 Container Upload",
                             null != serviceCapabilities.KeystoreCapabilities && serviceCapabilities.KeystoreCapabilities.PKCS8RSAKeyPairUploadSpecified && serviceCapabilities.KeystoreCapabilities.PKCS8RSAKeyPairUpload);
                LogAndNotify(Feature.PKCS12CertificateWithRSAPrivateKeyUpload,
                             "PKCS#12 Container Upload",
                             null != serviceCapabilities.KeystoreCapabilities && serviceCapabilities.KeystoreCapabilities.PKCS12CertificateWithRSAPrivateKeyUploadSpecified && serviceCapabilities.KeystoreCapabilities.PKCS12CertificateWithRSAPrivateKeyUpload);

                if (serviceCapabilities.KeystoreCapabilities.MaximumNumberOfKeysSpecified)
                {
                    MaximumNumberOfKeys = serviceCapabilities.KeystoreCapabilities.MaximumNumberOfKeys;
                    FeatureChecked(Feature.MaximumNumberOfKeys, true);
                }

                if (serviceCapabilities.KeystoreCapabilities.MaximumNumberOfCertificatesSpecified)
                {
                    MaximumNumberOfCertificates = serviceCapabilities.KeystoreCapabilities.MaximumNumberOfCertificates;
                    FeatureChecked(Feature.MaximumNumberOfCertificates, true);
                }
        ...
        void DefineCredentialCapabilities(Service[] services, DeviceServiceCapabilities capabilities)
        {
            BeginStep("Define Credential support");
            bool _csSupported = false;
            if (services != null)
            {
                _csSupported = services.Any(s => s.Namespace != null && s.Namespace.ToLower() == OnvifService.CREDENTIAL_SERVICE);

                if (_csSupported)
                {
                    Service asService = services.OrderByDescending(s => s.Version == null ? 0 : s.Version.Major).
                                                 ThenByDescending(s => s.Version == null ? 0 : s.Version.Minor).
                                                 FirstOrDefault(s => s.Namespace.ToLower() == OnvifService.CREDENTIAL_SERVICE);
                    _credentialServiceAddress = asService.XAddr;
                }
            }
            LogAndNotify(Feature.Credential, "Credential", _csSupported);
            StepPassed();
            ...
            if (_csSupported)
            {
                ...
                try
                {
                    serviceCapabilities = GetCredentialServiceCapabilities();
                }
                catch (Exception e)
                {
                    StepFailed(e);
                ...
                if (null == serviceCapabilities)
                {
                    LogStepEvent(string.Format("Get Credential service capabilities failed. Unable to determine whether features{0}"
                                               + "    'Credential Validity',{0}"
                                               + "     'Credential Access Profile Validity',{0}"
                                               + "     'pt:PIN',{0}"
                                               + "     'pt:Fingerprint',{0}"
                                               + "     'pt:Face',{0}"
                                               + "     'pt:Iris',{0}"
                                               + "     'pt:Vein',{0}"
                                               + "     'Validity Supports Time Value',{0}"
                                               + "     'Reset Antipassback Violation',{0}"
                                               + " are supported.", Environment.NewLine));
                    SetUndefined(Feature.CredentialValidity);
                    SetUndefined(Feature.CredentialAccessProfileValidity);
                    SetUndefined(Feature.PtCard);
                    SetUndefined(Feature.PtFace);
                    SetUndefined(Feature.PtFingerprint);
                    SetUndefined(Feature.PtIris);
                    SetUndefined(Feature.PtPIN);
                    SetUndefined(Feature.PtVein);
                    SetUndefined(Feature.ValiditySupportsTimeValue);
                    SetUndefined(Feature.ResetAntipassbackViolation);

                    LogStepEvent("");
                    return;
                }

                LogAndNotify(Feature.CredentialValidity,
                             "Credential Validity",
                             serviceCapabilities.CredentialValiditySupported);

                LogAndNotify(Feature.CredentialAccessProfileValidity,
                             "Credential Access Profile Validity",
                             serviceCapabilities.CredentialAccessProfileValiditySupported);

                LogAndNotify(Feature.ResetAntipassbackViolation,
                             "Reset Antipassback Violation",
                             serviceCapabilities.ResetAntipassbackSupported);

                LogAndNotify(Feature.ValiditySupportsTimeValue,
                             "Validity Supports Time Value",
                             serviceCapabilities.ValiditySupportsTimeValue);

                LogAndNotify(Feature.PtCard,
                             "pt:Card",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:Card"));

                LogAndNotify(Feature.PtCard,
                             "pt:Card",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:Card"));

                LogAndNotify(Feature.PtFace,
                             "pt:Face",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:Face"));

                LogAndNotify(Feature.PtFingerprint,
                             "pt:Fingerprint",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:Fingerprint"));

                LogAndNotify(Feature.PtIris,
                             "pt:Iris",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:Iris"));

                LogAndNotify(Feature.PtPIN,
                             "pt:PIN",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:PIN"));

                LogAndNotify(Feature.PtVein,
                             "pt:Vein",
                             serviceCapabilities.SupportedIdentifierType != null && serviceCapabilities.SupportedIdentifierType.Contains("pt:Vein"));

                LogStepEvent("");
            }
            else
                TransmitUnsupported(Feature.Credential);
        }

        void DefineScheduleCapabilities(Service[] services, DeviceServiceCapabilities capabilities)
        {
            BeginStep("Define Schedule support");
            bool _ssSupported = false;
            if (services != null)
            {
                _ssSupported = services.Any(s => s.Namespace != null && s.Namespace.ToLower() == OnvifService.SCHEDULE_SERVICE);

                if (_ssSupported)
                {
                    Service ssService = services.OrderByDescending(s => s.Version == null ? 0 : s.Version.Major).
                                                 ThenByDescending(s => s.Version == null ? 0 : s.Version.Minor).
                                                 FirstOrDefault(s => s.Namespace.ToLower() == OnvifService.SCHEDULE_SERVICE);
                    _scheduleServiceAddress = ssService.XAddr;
                }
            }
            LogAndNotify(Feature.Schedule, "Schedule", _ssSupported);
            StepPassed();
            ...
            if (_ssSupported)
            {
                ...
                try
                {
                    serviceCapabilities = GetScheduleServiceCapabilities();
                }
                catch (Exception e)
                {
                    StepFailed(e);
                ...
                if (null == serviceCapabilities)
                {
                    LogStepEvent(string.Format("Get Schedule service capabilities failed. Unable to determine whether features{0}"
                                               + "    'Extended Recurrence',{0}"
                                               + "    'Special Days',{0}"
                                               + "    'State Reporting',{0}"
                                               + " are supported.", Environment.NewLine));
                    LogStepEvent("");
                    SetUndefined(Feature.ExtendedRecurrence);
                    SetUndefined(Feature.SpecialDays);
                    SetUndefined(Feature.StateReporting);
                    return;
                }

                LogAndNotify(Feature.ExtendedRecurrence,
                             "Extended Recurrence",
                             serviceCapabilities.ExtendedRecurrenceSupported);

                LogAndNotify(Feature.SpecialDays,
                             "Special Days",
                             serviceCapabilities.SpecialDaysSupported);

                LogAndNotify(Feature.StateReporting,
                             "State Reporting",
                             serviceCapabilities.StateReportingSupported);

                LogStepEvent("");
            }
            else
                TransmitUnsupported(Feature.Schedule);
        }
                                  ...
                                  bool recordingControlSupported)
        {
            if (null == _eventsServiceAddress)
            {
                ...
                GetMonitoringEventTopics().Values.ToList().ForEach(feature => FeatureChecked(feature, false));

                return;
            }

            var eventTopics = GetMonitoringEventTopics();
            if (accessControlSuppported)
                eventTopics = eventTopics.Union(GetAccessControlEventTopics()).ToDictionary(e => e.Key, e => e.Value);
            if (doorControlSupported)
        ...
        void DefineEventServiceFeatures(bool getServicesSupported, bool eventServiceSupported, TestTool.Proxies.Event.EventServiceCapabilities eventServiceEventServiceCapabilities)
        {
            if (getServicesSupported)
            {
                if (eventServiceSupported && null != eventServiceEventServiceCapabilities)
                {
                    bool persistentNotificationStorageSupported = eventServiceEventServiceCapabilities.PersistentNotificationStorageSpecified && eventServiceEventServiceCapabilities.PersistentNotificationStorage;
                    ...
                    bool wsBasicNotificationSupported = !eventServiceEventServiceCapabilities.MaxNotificationProducersSpecified
                                                        || eventServiceEventServiceCapabilities.MaxNotificationProducersSpecified && eventServiceEventServiceCapabilities.MaxNotificationProducers > 0;
                    bool maxPullPointSupported = eventServiceEventServiceCapabilities.MaxPullPointsSpecified;
                    ...
                    if (maxPullPointSupported)
                        MaxPullPoints = eventServiceEventServiceCapabilities.MaxPullPoints;
