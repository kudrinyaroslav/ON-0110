...
{
    /// <summary>
    /// Holds base notification filter information
    /// </summary>
    public class FilterInfo
    {
        /// <summary>
        /// Filter ready to be sent.
        /// </summary>
        ...
        /// <summary>
        /// MessageDescription (copied from TopicSet).
        /// </summary>
        public XmlElement MessageDescription { get; set; }
    }
    ...
    public class EventTest : EventBaseTest
    {
        ...
        protected string _eventTopic;

        protected string _topicNamespaces;
        ...
        protected EventTest(TestLaunchParam param)
            : base(param)
        {
            _eventTopic = param.EventTopic;
            _topicNamespaces = param.TopicNamespaces;
        ...
        protected  void ReleaseSubscriptionManager(int timeout)
        {
            ...
            if (timeout > 0)
            {
                ...
                try
                {
                    ...
                    {
                        if (_subscriptionManagerClient.InnerChannel.State == CommunicationState.Faulted)
                        {
                            _subscriptionManagerClient = null;
                            LogStepEvent("Reference to Subscription Manager is incorrect");
                        }
                        else
                        {
            ...
            {
                LogStepEvent("Timeout has already expired");
            }
        ...
        protected bool ValidateBaseFault(BaseFaultType detail, out string reason)
        {
            ...
            if (timestampElement != null)
            {
                ...
                if (!EventServiceUtils.IsValidXsdDateTime(timestamp))
                {
        ...
        protected DateTime GetDateTimeFromFault<T>(FaultException<T> fault, string localName, string nameSpace)
...
#endif
        /// <summary>
        /// If FaultException object is not a derived class, it means we got unexpected exception
        /// that is not described using System.ServiceModel.FaultContractAttribute(or not fully meet those definitions)
        /// </summary>
        /// <param name="fault"></param>
        /// <param name="reason"></param>
        /// <returns></returns>
        protected bool validateFaultExceptionType(FaultException fault, out string reason)
        {
            bool flag = true;
            reason = string.Empty;
            //if (!fault.GetType().IsSubclassOf(typeof(FaultException)))
            if (!fault.Code.IsPredefinedFault)
            {
                reason += "FaultException of unknown type!\n";
                flag = false;
            }
            const string expectedFaultAction = "http://www.w3.org/2005/08/addressing/soap/fault";
            if (string.IsNullOrEmpty(fault.Action))
            {
                if (null == fault.Action)
                {

                    reason += string.Format("No Action element from namespace {0}. Expected element with value: {1}.\n",
                                            MessageVersion.Soap12WSAddressing10.Addressing, //SOAP messages should conform this version
                                            expectedFaultAction);
                    flag = false;
                }
                else
                {
                    reason += string.Format("Empty Action element from namespace {0}. Expected value: {1}.\n",
                                            MessageVersion.Soap12WSAddressing10.Addressing,
                                            //SOAP messages should conform this version
                                            expectedFaultAction);
                    flag = false;
                }
            }
            else if (expectedFaultAction != fault.Action)
            {
                if ("<DefaultAction>" != fault.Action)
                {
                    reason += string.Format("Invalid value for Action element. Expected: {0}. Actual: {1}.\n", expectedFaultAction, fault.Action);
                    flag = false;
                }
            }
