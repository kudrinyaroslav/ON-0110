...
namespace TestTool.Tests.TestCases.Base
{
    public class MediaTest : BaseServiceTest<Media, MediaClient>
    {
        ...
        protected DeviceClient GetDeviceClient()
        {
            ...
            TestTool.Proxies.Onvif.DeviceClient device = new TestTool.Proxies.Onvif.DeviceClient(binding, new EndpointAddress(CameraAddress));
        ...
        protected Profile CreateProfileBackupCopy(Profile p)
        {
            Profile copy = new Profile();
            copy.AudioEncoderConfiguration = p.AudioEncoderConfiguration;
            copy.AudioSourceConfiguration = p.AudioSourceConfiguration;
            copy.@fixed = p.@fixed;
            copy.fixedSpecified = p.fixedSpecified;
            copy.MetadataConfiguration = p.MetadataConfiguration;
            copy.Name = p.Name;
            copy.PTZConfiguration = p.PTZConfiguration;
            copy.token = p.token;
            copy.VideoAnalyticsConfiguration = p.VideoAnalyticsConfiguration;
            copy.VideoEncoderConfiguration = p.VideoEncoderConfiguration;
            copy.VideoSourceConfiguration = p.VideoSourceConfiguration;
        ...
        /// <param name="profile">Token of profile</param>
        /// <param name="configuration">Token of configuration</param>
        protected void AddAudioOutputConfiguration(string profile, string configuration)
        {
            RunStep(() => { Client.AddAudioOutputConfiguration(profile, configuration); },
                 string.Format(Resources.StepAddAudioOutputConfig_Format, configuration, profile));
        ...
        /// <param name="profile">Token of profile</param>
        /// <param name="configuration">Token of configuration</param>
        protected void AddAudioDecoderConfiguration(string profile, string configuration)
        {
            RunStep(() => { Client.AddAudioDecoderConfiguration(profile, configuration); },
                 string.Format(Resources.StepAddAudioDecoderConfig_Format, configuration, profile));

            DoRequestDelay();
        }

        protected void RemoveAudioDecoderConfiguration(string profile)
        {
            RunStep(() => { Client.RemoveAudioDecoderConfiguration(profile); }, string.Format(Resources.StepRemoveAudioDecoderConfig_Format, profile));
            DoRequestDelay();
        }
        ...
        /// </summary>
        /// <param name="profile">Token of profile</param>
        protected void RemoveVideoAnalyticsConfiguration(string profile)
        {
            RunStep(() => { Client.RemoveVideoAnalyticsConfiguration(profile); },
                string.Format(Resources.StepRemoveVideoAnalythicsConfig_Format, profile));
        ...
        /// <param name="profile">Token of profile</param>
        /// <returns>Array of audio source configurations</returns>
        protected AudioDecoderConfiguration[] GetCompatibleAudioDecoderConfigurations(string profile)
        {
            AudioDecoderConfiguration[] configs = null;
            RunStep(() => { configs = Client.GetCompatibleAudioDecoderConfigurations(profile); },
                "Get compatible audio decoder configurations");
            DoRequestDelay();
            return configs;
        }
        ...
        /// <param name="profileToken">Profile token.</param>
        /// <returns>Audio encoder configuration options</returns>
        protected AudioDecoderConfigurationOptions GetAudioDecoderConfigurationOptions(string configurationToken, string profileToken)
        {
        ...
        /// <param name="stepName">Step name to be used.</param>
        /// <returns>Audio encoder configuration options</returns>
        protected AudioDecoderConfigurationOptions GetAudioDecoderConfigurationOptions(string configurationToken, string profileToken, string stepName)
        {
            AudioDecoderConfigurationOptions options = null;
            RunStep(() => { options = Client.GetAudioDecoderConfigurationOptions(configurationToken, profileToken); }, stepName);
            DoRequestDelay();
            return options;
        }
        ...
        protected bool EqualIPAddresses(IPAddress a, IPAddress b)
        {
            ...
            return (a.IPv4Address == b.IPv4Address) && (a.IPv6Address == b.IPv6Address) && (a.Type == b.Type);
        }
                ...
                case VideoEncoding.MPEG4:
                    {
                        ...
                        {
                            ok = false;
                            dump.AppendLine("MPEG4 settings are missing");
                        }
                    }
                    break;
                case VideoEncoding.H264:
                    {
                        ...
                        {
                            ok = false;
                            dump.AppendLine("H264 settings are missing");
                        }
                    }
                    break;
            }
        ...
        protected bool EqualConfigurations(AudioEncoderConfiguration a, AudioEncoderConfiguration b, out string reason)
        {
            ...
            if (a.Name != b.Name)
            {
                logger.AppendLine(Resources.ConfigsDiffNames_Text);
                ok = false;
            }
            if (a.token != b.token)
            {
                logger.AppendLine(Resources.ConfigsDiffTokens_Text);
                ok = false;
            }
            /*if (a.SessionTimeout != b.SessionTimeout)
            {
            ...
            if (a.Encoding != b.Encoding)
            {
                logger.AppendLine(Resources.ConfigsDiffEncodings_Text);
                ok = false;
            }
            //this is according to agreement with ONVIF WG - should not check multicast configuration
            /*if (!EqualMulticastConfiguration(a.Multicast, b.Multicast))
            ...
            {
                logger.AppendLine(Resources.ConfigsDiffSampleRates_Text);
                ok = false;
            }
            if (a.Bitrate != b.Bitrate)
            {
                logger.AppendLine(Resources.ConfigsDiffBitrates_Text);
                ok = false;
            }
            reason = logger.ToStringTrimNewLine();
            return ok;
        }
        ...
        /// <summary>
        /// Gets audio decoder configurations.
        /// </summary>
        /// <returns></returns>
        protected AudioDecoderConfiguration[] GetAudioDecoderConfigurations()
        {
            return GetAudioDecoderConfigurations("Get audio decoder configurations");
        }
        ...
        /// <returns></returns>
        protected AudioDecoderConfiguration[] GetAudioDecoderConfigurations(string stepName)
        {
            AudioDecoderConfiguration[] audioDecoderConfigurations = null;
            RunStep(() => { audioDecoderConfigurations = Client.GetAudioDecoderConfigurations(); }, stepName);
            DoRequestDelay();
            return audioDecoderConfigurations;
        }

        /// <summary>
        /// Gets single audio decoder configuration.
        /// </summary>
        /// <param name="token"></param>
        /// <returns></returns>
        protected AudioDecoderConfiguration GetAudioDecoderConfiguration(string token)
        {
            return GetAudioDecoderConfiguration(token, "Get audio decoder configuration");
        }
        ...
        /// <param name="stepName"></param>
        /// <returns></returns>
        protected AudioDecoderConfiguration GetAudioDecoderConfiguration(string token, string stepName)
        {
            AudioDecoderConfiguration audioDecoderConfiguration = null;
            RunStep(() => { audioDecoderConfiguration = Client.GetAudioDecoderConfiguration(token); }, stepName);
            DoRequestDelay();
        ...
        protected AudioOutput[] GetAudioOutputs()
        {
            ...
            RunStep(() => { audioOutputs = Client.GetAudioOutputs(); }, "GetAudioOutputs");
            DoRequestDelay();
            return audioOutputs;
        }
        ...
        protected AudioOutputConfiguration[] GetCompatibleAudioOutputConfigurations(string profileToken)
        {
            AudioOutputConfiguration[] Configurations = null;
            RunStep(() => { Configurations = Client.GetCompatibleAudioOutputConfigurations(profileToken); },
              "Get compatible audio output configurations");
            DoRequestDelay();
        ...
        protected void RemoveAudioOutputConfiguration(string profile)
        {
            RunStep(() => { Client.RemoveAudioOutputConfiguration(profile); }, string.Format(Resources.StepRemoveAudioOutputConfig_Format, profile));
            DoRequestDelay();
        }
        ...
        protected PTZConfiguration[] GetPtzConfigurations()
        {
            ...
            return CommonMethodsProvider.GetPtzConfigurations(this, client);
        }

        protected MediaServiceCapabilities GetServiceCapabilities()
        {
            return GetServiceCapabilities("Get Media Service capabilities");
        }

        protected MediaServiceCapabilities GetServiceCapabilities(string stepName)
        {
            MediaServiceCapabilities capabilities = null;
            RunStep(() => { capabilities = Client.GetServiceCapabilities(new GetServiceCapabilities()).Capabilities; }, stepName);
            DoRequestDelay();
            return capabilities;
        }

        protected Profile CreateProfileByAnnex3(string name, string token, out Profile deletedProfile, out Profile createdProfile, out Profile modifiedProfile)
        {
            deletedProfile = null;
            createdProfile = null;
            ...
            if (executeAnnex)
            {
                ...
                else
                {
                    foreach (Profile p in profiles)
                    {
                        profile = p;
                        modifiedProfile = p;
                        Profile modifiedProfileNeedToRestore = new Profile();
                        modifiedProfileNeedToRestore.VideoSourceConfiguration = null;
                        modifiedProfileNeedToRestore.VideoEncoderConfiguration = null;
                        modifiedProfileNeedToRestore.PTZConfiguration = null;
                        modifiedProfileNeedToRestore.MetadataConfiguration = null;
                        modifiedProfileNeedToRestore.VideoAnalyticsConfiguration = null;
                        modifiedProfileNeedToRestore.AudioSourceConfiguration = null;
                        modifiedProfileNeedToRestore.AudioEncoderConfiguration = null;
                        modifiedProfileNeedToRestore.Extension = new ProfileExtension();
                        modifiedProfileNeedToRestore.Extension.AudioDecoderConfiguration = null;
                        modifiedProfileNeedToRestore.Extension.AudioOutputConfiguration = null;
                        try
                        {
                            if (p.Extension != null && p.Extension.AudioOutputConfiguration != null)
                            {
                                RemoveAudioOutputConfiguration(p.token);
                                modifiedProfileNeedToRestore.Extension.AudioOutputConfiguration = modifiedProfile.Extension.AudioOutputConfiguration;
                            }
                            if (p.Extension != null && p.Extension.AudioDecoderConfiguration != null)
                            {
                                RemoveAudioDecoderConfiguration(p.token);
                                modifiedProfileNeedToRestore.Extension.AudioDecoderConfiguration = modifiedProfile.Extension.AudioDecoderConfiguration;
                            }
                            if (p.VideoSourceConfiguration != null)
                            {
                                RemoveVideoSourceConfiguration(p.token);
                                modifiedProfileNeedToRestore.VideoSourceConfiguration = modifiedProfile.VideoSourceConfiguration;
                            }
                            if (p.VideoEncoderConfiguration != null)
                            {
                                RemoveVideoEncoderConfiguration(p.token);
                                modifiedProfileNeedToRestore.VideoEncoderConfiguration = modifiedProfile.VideoEncoderConfiguration;
                            }
                            if (p.PTZConfiguration != null)
                            {
                                RemovePTZConfiguration(p.token);
                                modifiedProfileNeedToRestore.PTZConfiguration = modifiedProfile.PTZConfiguration;
                            }
                            if (p.MetadataConfiguration != null)
                            {
                                RemoveMetadataConfiguration(p.token);
                                modifiedProfileNeedToRestore.MetadataConfiguration = modifiedProfile.MetadataConfiguration;
                            }
                            if (p.VideoAnalyticsConfiguration != null)
                            {
                                RemoveVideoAnalyticsConfiguration(p.token);
                                modifiedProfileNeedToRestore.VideoAnalyticsConfiguration = modifiedProfile.VideoAnalyticsConfiguration;
                            }
                            if (p.AudioSourceConfiguration != null)
                            {
                                RemoveAudioSourceConfiguration(p.token);
                                modifiedProfileNeedToRestore.AudioSourceConfiguration = modifiedProfile.AudioSourceConfiguration;
                            }
                            if (p.AudioEncoderConfiguration != null)
                            {
                                RemoveAudioEncoderConfiguration(p.token);
                                modifiedProfileNeedToRestore.AudioEncoderConfiguration = modifiedProfile.AudioEncoderConfiguration;
                            }
                        }
                        finally
                        {
                            modifiedProfile.VideoSourceConfiguration = modifiedProfileNeedToRestore.VideoSourceConfiguration;
                            modifiedProfile.VideoEncoderConfiguration = modifiedProfileNeedToRestore.VideoEncoderConfiguration;
                            modifiedProfile.PTZConfiguration = modifiedProfileNeedToRestore.PTZConfiguration;
                            modifiedProfile.MetadataConfiguration = modifiedProfileNeedToRestore.MetadataConfiguration;
                            modifiedProfile.VideoAnalyticsConfiguration = modifiedProfileNeedToRestore.VideoAnalyticsConfiguration;
                            modifiedProfile.AudioSourceConfiguration = modifiedProfileNeedToRestore.AudioSourceConfiguration;
                            modifiedProfile.AudioEncoderConfiguration = modifiedProfileNeedToRestore.AudioEncoderConfiguration;
                            modifiedProfile.Extension = modifiedProfile.Extension ?? new ProfileExtension();
                            modifiedProfile.Extension.AudioOutputConfiguration = modifiedProfileNeedToRestore.Extension.AudioOutputConfiguration;
                            modifiedProfile.Extension.AudioDecoderConfiguration = modifiedProfileNeedToRestore.Extension.AudioDecoderConfiguration;
                        }

                        profile = GetProfile(p.token);
                        break;
                    }
                }
            }
        ...
        protected void RestoreProfileByAnnex3(Profile deletedProfile, Profile createdProfile, Profile modifiedProfile)
        {
            ...
            {
                RestoreProfile(profileToRestore, remark, isModified);
            }
        }
        protected void RestoreProfile(Profile profile)
        {
            RestoreProfile(profile, "Restore profile used for test", true);
        }

        protected void RestoreProfile(Profile profile, string remark, bool isModified)
        {
            LogTestEvent(remark + Environment.NewLine);

            if (!isModified)
            {
                Profile p = CreateProfile(profile.Name, profile.token);
                profile.token = p.token;

            }

            if (profile.Extension != null && profile.Extension.AudioOutputConfiguration != null)
            {
                AddAudioOutputConfiguration(profile.token, profile.Extension.AudioOutputConfiguration.token);
            }

            if (profile.Extension != null && profile.Extension.AudioDecoderConfiguration != null)
            {
                AddAudioDecoderConfiguration(profile.token, profile.Extension.AudioDecoderConfiguration.token);
            }
