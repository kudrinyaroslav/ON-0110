...
namespace TestTool.Tests.TestCases.TestSuites.DeviceManagement
{
    class DeviceManagementNetworkTestSuiteEx : DeviceManagementIPTestSuite
    {
        ...
        NetworkInterface TurnOnDhcpIpv4(out bool networkSettingsChanged)
        {
            ...
            SetIPConfiguration(ni.token, configuration, out networkSettingsChanged);

            return ni;
        }

        NetworkInterface TurnOffDhcpIpv4(out bool networkSettingsChanged)
        {
            networkSettingsChanged = false;
        ...
        NetworkInterface TurnOffDhcpIpv6(out bool networkSettingsChanged)
        {
            networkSettingsChanged = false;
            ...
            RunTest<DNSInformation>(() =>
                                    {
                                        ...
                                        ni = TurnOffDhcpIpv4(out restoreNetworkSettings);
                                    ...
                                    () =>
                                    {
                                        ...
                                        testInformation.DNSManual = new IPAddress[]{ new IPAddress(){IPv4Address = _environmentSettings.DnsIpv4, Type = IPType.IPv4} };
                                        ...
                                        LogStepEvent(string.Format("FromDHCP: expected - {0}, actual - {1}", testInformation.FromDHCP, actualInformation.FromDHCP));
                                        ...
                                        if (actualInformation.DNSManual.Any(A => A.Type == expectedType && A.IPv4Address == expectedAddress))
                                        {
                                            bEquals = true;
                                        }
                                        else
                                        ...
                                        LogStepEvent(string.Format("DNSManual: expected - {0} should be presented, actual - {1}", expectedAddress, actualIpDescription));
                                        ...
                                        LogStepEvent(string.Format("DNSFromDHCP: expected - {0}, actual - {1}", expectedDescription, actualIpDescription));
                                    ...
                                    (originalInformation) =>
                                    {
                                        ...
                                        if (restoreNetworkSettings && ni.IPv4.Config.DHCP)
                                        {
            ...
            RunTest<DNSInformation>(() =>
                                    {
                                        ...
                                        ni = TurnOffDhcpIpv6(out restoreNetworkSettings);
                                    ...
                                    () =>
                                    {
                                        ...
                                        var testInformation = new DNSInformation();
                                        testInformation.FromDHCP = false;
                                        testInformation.DNSManual =
                                            new IPAddress[]{ new IPAddress(){IPv6Address = _environmentSettings.DnsIpv6, Type = IPType.IPv6} };
                                        ...
                                        if (actualInformation.DNSManual.Any(A => A.Type == expectedType && A.IPv6Address == expectedAddress))
                                        {
                                            bEquals = true;
                                        }
                                        else
                                    ...
                                    {
                                        SetDnsConfiguration(originalInformation, "Restore DNS configuration");
                                        if (restoreNetworkSettings && ni.IPv6.Config.DHCP != IPv6DHCPConfiguration.Off)
                                        {
                                            // restore network interface configuration
                                            RestoreNetworkInterface(ni.token, ni);
                                        }
            ...
            RunTest<DNSInformation>(() =>
                                    {
                                        ...
                                        ni = TurnOnDhcpIpv4(out restoreNetworkSettings);
                                    ...
                                    () =>
                                    {
                                        ...
                                        if (actualInformation.DNSManual != null && actualInformation.DNSManual.Any())
                                        {
                                            bEquals = false;
                                        }
                                        else
                                        ...
                                        LogStepEvent(string.Format("DNSManual: expected - {0}, actual - {1}", expectedAddress, actualIpDescription));
        ...
        public void SetInvalidDnsConfigurationTest()
        {
            ...
            RunTest<NetworkInterface>(() =>
                                      {
                                          NetworkInterface ni = TurnOffDhcpIpv4(out restoreNetworkSettings);
                                          return ni;
                                      },
                                      () =>
                                      {
                                          var testInformation = new DNSInformation();
                                          testInformation.FromDHCP = false;
                                          testInformation.DNSManual = new IPAddress[] { new IPAddress() {IPv4Address = "10.1.1", Type = IPType.IPv4} };

                                          string reason = string.Empty;
                                          RunStep(() => Client.SetDNS(testInformation.FromDHCP, testInformation.SearchDomain, testInformation.DNSManual),
                                          ...
                                          bool bFound = (actualInformation.DNSManual != null && actualInformation.DNSManual.Length > 0) &&
                                                        actualInformation.DNSManual.Any(IP => IP.IPv4Address == testInformation.DNSManual[0].IPv4Address);
        ...
        public void SetInvalidIpv6DnsConfigurationTest()
        {
            ...
            RunTest<NetworkInterface>(() =>
                                      {
                                          NetworkInterface ni = TurnOffDhcpIpv6(out restoreNetworkSettings);
                                          return ni;
                                      },
                                      () =>
                                      {
                                          var testInformation = new DNSInformation();
                                          testInformation.FromDHCP = false;
                                          testInformation.DNSManual = new IPAddress[] { new IPAddress() {IPv6Address = "FF02:1", Type = IPType.IPv6} };

                                          string reason = string.Empty;
                                          RunStep(() => Client.SetDNS(testInformation.FromDHCP, testInformation.SearchDomain,
                                                                      testInformation.DNSManual),
                                                  "Set DNS configuration - negative test",
                                          ...
                                          bool bFound = (actualInformation.DNSManual != null && actualInformation.DNSManual.Length > 0) &&
                                                        actualInformation.DNSManual.Any(IP => IP.IPv4Address == testInformation.DNSManual[0].IPv6Address);
        ...
        public void SetNtpConfigurationTest()
        {
            ...
            RunTest<NTPInformation>(() =>
                                    ...
                                    () =>
                                    {
                                        ...
                                        if (currentInformation.NTPManual.Any(A => A.Type == type && A.IPv4Address == expectedAddress))
                                        {
                                            bEquals = true;
                                        }
                                        else
                                        ...
                                        string expectedDescription = string.Format("{0} should be presented", testInformation.NTPManual[0].IPv4Address);

                                        LogStepEvent(string.Format("NTPManual: expected - {0}, actual - {1}", expectedDescription, actualIpDescription));
                                        ...
                                        LogStepEvent(string.Format("NTPFromDHCP: expected - {0}, actual - {1}", expectedDescription, actualIpDescription));
        ...
        public void SetNtpIpv6ConfigurationTest()
        {
            ...
            RunTest<NTPInformation>(() =>
                                    ...
                                    () =>
                                    {
                                        ...
                                        LogStepEvent(string.Format("FromDHCP: expected - {0}, actual - {1}", testInformation.FromDHCP, currentInformation.FromDHCP));
                                        ...
                                        if (currentInformation.NTPManual.Any(A => A.Type == type && A.IPv6Address == expectedAddress))
                                        {
                                            bEquals = true;
                                        }
                                        else
                                        ...
                                        string expectedDescription = string.Format("{0} should be presented", testInformation.NTPManual[0].IPv6Address);
        ...
        public void SetNtpConfigurationDnsFromDHCPTest()
        {
            ...
            RunTest<NTPInformation>(() =>
                                    ...
                                    () =>
                                    {
                                        ...
                                        if (actualInformation.NTPManual != null && actualInformation.NTPManual.Any())
                                        {
                                            bEquals = false;
                                        }
                                        else
                                        ...
                                        LogStepEvent(string.Format("NTPManual: expected - {0}, actual - {1}", expectedAddress, actualIpDescription));
            ...
            RunTest<NetworkInterface>(() =>
                                      ...
                                      () =>
                                      {
                                          ...
                                          bool bFound = (currentInformation.NTPManual != null) &&
                                                        (currentInformation.NTPManual.FirstOrDefault(IP => IP.IPv4Address == "10.1.1") != null);
        ...
        public void SetInvalidIpv6NtpConfigurationTest()
        {
            ...
            RunTest<NetworkInterface>(() =>
                                      ...
                                      () =>
                                      {
                                          ...
                                          bool bFound = (currentInformation.NTPManual != null) &&
                                                        (currentInformation.NTPManual.FirstOrDefault(IP => IP.IPv6Address == "FF02:1") != null);
