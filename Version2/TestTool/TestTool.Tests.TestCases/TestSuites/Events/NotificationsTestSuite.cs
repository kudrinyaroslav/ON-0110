...
//                    topicSet);
        ...
        [Test(Name = "REALTIME PULLPOINT SUBSCRIPTION – PULLMESSAGES AS KEEP-ALIVE",
            ...
            FunctionalityUnderTest = new Functionality[] { Functionality.CreatePullPointSubscription, Functionality.PullMessages })]
        public void CreateSubscriptionPullmessagesAsKeepAliveTest()
        {
            ...
            RunTest(() =>
                    {
                        ...
                        subscription = new SubscriptionHandler(this, false, GetEventServiceAddress());
                        subscription.SetPolicy(new EventsVerifyPolicy(false){ VerifyMessagesPresence = false });
                        subscription.Subscribe(null, -1);

                        RunStep(() => Sleep(1000), "1 second after CreatePullPointSubscription");

                        Assert(subscription.CurrentTime < subscription.TerminationTime,
                               "Termination Time is less or equal to Current Time in CreatePullPointSubscription response",
                               "Validating Current Time and Termination Time in CreatePullPointSubscription response");


                        //DateTime quiteLongTerminationTime = subscription.CurrentTime.AddYears(1);
                        //if (subscription.TerminationTime >= quiteLongTerminationTime)
                        //{
                        //    LogStepEvent("WARNING: Subscription Termination Time is too long. Pass the test.");
                        //    LogStepEvent("");
                        //    return;
                        //}

                        if (!propertyTopics.Any())
                        {
                            bool bContinue = Operator.GetOkCancelAnswer("No property events found. You'll have to trigger an event manually.");

                            Assert(bContinue, "Operator cancelled the test", "Check if test should be continued");
                        }

                        Dictionary<NotificationMessageHolderType, XmlElement> messages = null;
                        var timeout = XmlConvert.ToString((subscription.TerminationTime - subscription.CurrentTime).Add(new TimeSpan(0, 0, 0, 20)));

                        subscription.PullMessagesRequestTimeout = timeout;
                        subscription.NotificationResponeWaitTimeout = OperationDelay;

                        DateTime terminationTime;
                        var currentTime = subscription.GetMessages(1, () => {}, "(User action required to trigger event)", out terminationTime, out messages);

                        var validateTerminationTimeIncreasing = (subscription.PullMessagesRequestTimeout == timeout);

                        //quiteLongTerminationTime = currentTime.AddMinutes(10);
                        //if (terminationTime >= quiteLongTerminationTime)
                        //{
                        //    LogStepEvent("WARNING: Subscription Termination Time isn't increased after PullMessages request");
                        //    return;
                        //}

                        var flag = true;
                        var log = new StringBuilder("Validation failed!");
                        if (terminationTime <= currentTime)
                        {
                            flag = false;
                            log.AppendLine();
                            log.Append("In PullMessages response Termination Time is less or equal to Current Time");
                        }
                        if (validateTerminationTimeIncreasing)
                        {
                            if (terminationTime <= subscription.TerminationTime)
                            {
                                flag = false;
                                log.AppendLine();
                                log.Append("Termination Time in PullMessages response is less or equal to Termination Time of subscription");
                            }
                        }
                        else
                        {
                            if (terminationTime < subscription.TerminationTime)
                            {
                                flag = false;
                                log.AppendLine();
                                log.Append("Termination Time in PullMessages response is less than Termination Time of subscription");
                            }
                            else
                            {
                                if (terminationTime == subscription.TerminationTime)
                        ...
                        Assert(flag,
                               log.ToString(),
                               "Validating Current Time and Termination Time in PullMessages response");
                    },
