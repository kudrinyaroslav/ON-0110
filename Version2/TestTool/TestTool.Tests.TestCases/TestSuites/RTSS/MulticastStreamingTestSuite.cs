...
namespace TestTool.Tests.TestCases.TestSuites.RTSS
{
    ...
    class MulticastStreamingTestSuite : MediaConfigurationTestSuiteBase
    {
            ...
            RunTest(() =>
                    {
                        ...
                        foreach (AudioEncoderConfiguration aec in encoderConfigurations)
                        {
                            ...
                            // streaming setup
                            //VideoUtils.AdjustVideo(_videoForm, null, null, _messageTimeout, 
                            //                       TransportProtocol.UDP, StreamType.RTPMulticast, null, null);

                            string token = profile.token;
                            RunStep(() => Client.StartMulticastStreaming(token),
            ...
            RunTest(() =>
                    {
                       ...
                       CheckVideoEncoderConfigurationsList(encoderConfigurations);
                        ...
                        foreach (VideoEncoderConfiguration vec in encoderConfigurations)
                        {
                            ...
                            RunStep(() => 
                                    {
                                        LogStepEvent(string.Format("Start streaming to {0}:{1} (video encoder configuration settings), {2}:{3} (profile settings)",
        ...
        private void StartMedia(bool useAudio, bool useVideo, Profile profile)
        {
          if (NewGenVideo == null)
          {
        ...
        private void StartVideo(Profile profile, MediaUri Uri)
        {
          if (NewGenVideo == null)
          {
            NewGenVideo = new VideoContainer2();
          }
          VideoUtils.AdjustGeneral2(NewGenVideo, _username, _password, _messageTimeout, TransportProtocol.UDP, StreamType.RTPMulticast, Uri);
          VideoUtils.AdjustVideo2(NewGenVideo, profile.VideoEncoderConfiguration);
