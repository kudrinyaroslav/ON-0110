...
namespace TestTool.Tests.TestCases.TestSuites
{
    class RTSSTestSuite : MediaTest, IVideoFormEvent
    {
        ...
        private bool AdjustResolution(VideoEncoderConfiguration config, VideoResolution[] ResolutionsAvailable)
        {
          if (null != ResolutionsAvailable && ResolutionsAvailable.Length > 0)
          {
            foreach (VideoResolution vr in ResolutionsAvailable) 
            {
              if (vr == config.Resolution)
              {
                return true;
              }
            }
            config.Resolution = ResolutionsAvailable[0];
            return true;
          }
          return false;
        }
            ...
            VideoEncoderConfigurationOptions options)
        {
            ...
            if (encoding == VideoEncoding.JPEG)
            {
                ...
                {
                  resolutionAdjusted = AdjustResolution(config, options.JPEG.ResolutionsAvailable);
                }
                ...
                if (options.Extension != null)
                {
                    if (options.Extension.JPEG != null)
                    {
                        ...
                        {
                          AdjustResolution(config, options.Extension.JPEG.ResolutionsAvailable);
                        }
                    }
                }
            }
            if (encoding == VideoEncoding.MPEG4)
            {
                ...
                {
                  resolutionAdjusted = AdjustResolution(config, options.MPEG4.ResolutionsAvailable);
                }
                ...
                if (options.Extension != null)
                {
                    if (options.Extension.MPEG4 != null)
                    {
                        ...
                        if (!resolutionAdjusted)
                        {
                          AdjustResolution(config, options.Extension.MPEG4.ResolutionsAvailable);
                        }
                    }
                }
            }
            if (encoding == VideoEncoding.H264)
            {
                ...
                {
                  resolutionAdjusted = AdjustResolution(config, options.H264.ResolutionsAvailable);
                }
                ...
                if (options.Extension != null)
                {
                    if (options.Extension.H264 != null)
                    {
                        ...
                        if (!resolutionAdjusted)
                        {
                          AdjustResolution(config, options.Extension.H264.ResolutionsAvailable);
                        }
                    }
                }
            }
        ...
        {
          //TestFailed(new VideoException(Message));
        ...
        public void ValidateStreamSequence2(bool CheckAudio, bool CheckVideo)
        {
          NewGenVideo.EventSink = this;
          NewGenVideo.SetSequence(3);
          NewGenVideo.Connect();
        ...
        public void ValidateStreamSequence22()
        {
          NewGenVideo.EventSink = this;
          NewGenVideo.SetSequence(2, OperationDelay);
          NewGenVideo.Connect();
          NewGenVideo.SetupWindow();
          VideoIsOpened = true;
        ...
        protected void StartMulticastStreaming2(
            out Profile profile,
            VideoEncoding? videoEncoding,
            AudioEncoding? audioEncoding,
            MediaConfigurationChangeLog changeLog)
        {
          ...
          VideoUtils.AdjustGeneral2(
              NewGenVideo, null, null, _messageTimeout, TransportProtocol.UDP,
              StreamType.RTPMulticast, null);//, profile.VideoEncoderConfiguration);

          if (useVideo)
          {
            VideoUtils.AdjustVideo2(NewGenVideo, profile.VideoEncoderConfiguration);
          }
          if (useAudio)
          {
            VideoUtils.AdjustAudio2(NewGenVideo, profile.AudioEncoderConfiguration);
          }


          ValidateStreamSequence2(useAudio, useVideo);
        }

        protected void StopMulticastStreaming2(Profile profile)
        {
          VideoCleanup2();
          if (profile != null)
          {
        ...
        protected void AudioMulticastStreamingSetup2(
            ref Profile profile, AudioEncoderConfigurationOptions audioOptions,
            AudioEncoding encoding, TransportProtocol protocol,
            StreamType streamType, IPType? multicastAddressType,
            out bool started,
            MediaConfigurationChangeLog changeLog)
        {
          ...
          VideoUtils.AdjustGeneral2(
              NewGenVideo, null, null, _messageTimeout, TransportProtocol.UDP,
              StreamType.RTPMulticast, null);

          VideoUtils.AdjustAudio2(NewGenVideo, profile.AudioEncoderConfiguration);


          ValidateStreamSequence2(true, false);
        }
