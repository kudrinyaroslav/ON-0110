...
namespace TestTool.Tests.TestCases.TestSuites.Recording
{
    partial class RecordingControlRecordingTestSuite
    {
        ...
        protected string AutoCreationReceiver(string recordingToken, out string jobToken, out RecordingJobConfiguration jobConfiguration)
        {
            jobToken = null;
        ...
        bool CheckJobMessage(Event.NotificationMessageHolderType message, string operationType, string jobToken, IEnumerable<string> expectedStates)
        {
            if (message.Message.HasAttribute(OnvifMessage.PROPERTYOPERATIONTYPE) && !string.IsNullOrEmpty(operationType))
            {
            ...
            if (!expectedStates.Contains(dataSimpleItems[STATESIMPLEITEM]))
            {
                return false;
            }
            return true;
        ...
        {
            return CheckJobMessage(message, operationType, jobToken, new []{ state });
        }
        ...
        void ValidateJobState(RecordingJobStateInformation state, string recordingToken, IEnumerable<string> expectedGeneralStates,
                              string sourceToken = null, string expectedSourceState = null)
        {
            StringBuilder sb = new StringBuilder();
            bool ok = true;
            ...
            var expectedStatesString = expectedGeneralStates.Aggregate("", (s, s1) => s + ", " + s1).Trim(' ', ',');
            var expectedStatesMsg = string.Format(1 == expectedGeneralStates.Count() ? "'{0}'" : "one of [{0}]", expectedStatesString);
            if (!expectedGeneralStates.Contains(state.State))
            {
                ok = false;
                sb.AppendLine(string.Format("State is incorrect: expected {0}, actual {1}", expectedStatesMsg, state.State));
            }

            if (state.Sources != null && !string.IsNullOrEmpty(sourceToken) && !string.IsNullOrEmpty(expectedSourceState))
            {
                foreach (RecordingJobStateSource source in state.Sources.Where(e => sourceToken == e.SourceToken.Token))
                {
                    //if (source.State != null)
                    if (!string.IsNullOrEmpty(source.State) && source.State != expectedSourceState)
                    {
                        ok = false;
                        sb.AppendLine(string.Format("State for source with token '{0}' is incorrect: expected {1}, actual {2}", source.SourceToken.Token, expectedSourceState, source.State));
                    }
                }
            }
        ...
        bool ValidateJobStateSimple(RecordingJobStateInformation state, string recordingToken, 
                                                            IEnumerable<string> expectedGeneralStates,
                                                            string sourceToken = null, string expectedSourceState = null)
        {
            StringBuilder sb = new StringBuilder();
            bool ok = true;
            ...
            RunStep(() => 
            {
                ...
                {
                    ok = false;
                    LogStepEvent(string.Format("State is incorrect: expected {0}, actual {1}", expectedStatesMsg, state.State));
                }
                else
                {
                    ok = true;
            ...
            "Validate state of recording job");
        ...
        void ValidateJobState(RecordingJobStateInformation state, string recordingToken, string expectedState,
                              string sourceToken = null, string expectedSourceState = null)
        {
            ValidateJobState(state, recordingToken, new []{ expectedState }, sourceToken, expectedSourceState);
        }
        ...
        {
            //[18.02.2014] AKS: Ticket #539. Parameters Quality, BitrateLimit and FrameRateLimit might be adopted. So don't check them.
            StringBuilder logger = new StringBuilder();
            bool ok = true;
            if (newConf != null)
            {
                ...
                //{
                //    ok = false;
                //    logger.Append(string.Format("Quality is {0} but must be {1}{2}", newConf.Quality, conf.Quality, Environment.NewLine));
                //}
                if (newConf.Resolution != null)
                {
                    ...
                    {
                        ok = false;
                        logger.Append(string.Format("Resolution.Height is {0} but must be {1}{2}", newConf.Resolution.Height, conf.Resolution.Height, Environment.NewLine));
                    }
                    if (newConf.Resolution.Width != conf.Resolution.Width)
                    {
                        ok = false;
                        logger.Append(string.Format("Resolution.Width is {0} but must be {1}{2}", newConf.Resolution.Width, conf.Resolution.Width, Environment.NewLine));
                    }
                }
                else
                {
                    ok = false;
                    logger.Append(string.Format("VideoEncoderConfigurationResponse doesn't contain Resolution{0}", Environment.NewLine));
                }
                if (newConf.RateControl != null)
                {
                    newConf.RateControl.BitrateLimit = 1;
                    //if (newConf.RateControl.BitrateLimit != conf.RateControl.BitrateLimit)
                    //{
                    //    ok = false;
                    //    logger.Append(string.Format("RateControl.BitrateLimit is {0} but must be {1}{2}", newConf.RateControl.BitrateLimit, conf.RateControl.BitrateLimit, Environment.NewLine));
                    //}
                    //if (newConf.RateControl.FrameRateLimit != conf.RateControl.FrameRateLimit)
                    //{
                    //    ok = false;
                    //    logger.Append(string.Format("RateControl.FrameRateLimit is {0} but must be {1}{2}", newConf.RateControl.FrameRateLimit, conf.RateControl.FrameRateLimit, Environment.NewLine));
                    //}
                }
                else
                {
                    ok = false;
                    logger.Append(string.Format("VideoEncoderConfigurationResponse doesn't contain RateControl{0}", Environment.NewLine));
                }
            }
            else
            {
                ok = false;
                logger.Append(string.Format("Video encoder configuration wasn't returned{0}",Environment.NewLine)); 
            }
