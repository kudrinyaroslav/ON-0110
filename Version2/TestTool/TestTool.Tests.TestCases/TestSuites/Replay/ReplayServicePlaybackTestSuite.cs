...
namespace TestTool.Tests.TestCases.TestSuites
{
    ...
    class ReplayServicePlaybackTestSuite : Base.SearchTest, IVideoFormEvent
    {
        ...
        protected const int PACKET_LOG_INTERVAL_MS = 200;
        protected const int MIN_PACKETS_NUM_TO_SHOW = 5;
        ...
        protected bool m_PauseResponseReceived;
        protected DateTime m_PauseDateField;
            ...
            if (rtpPacket.Parse(message))
            {
                    ...
                    || rtpPacket.ForceShow)
                {
                    if (waitStreamStarted && !(_lastShownPacket.ContainsKey(payload) && (_lastShownPacket[payload] == rtpPacket)))
                    {
                        ...
                        LogStepEvent(rtpPacket.ToString() + string.Format(", Captured as # {0}", rtpPackets.Count + 1));
                        ++packetsShowed;
                    }
                }
        ...
        protected bool HadleMessagePause(string message, DateTime? pauseTime)
        {
            ...
            if (match.Success)
            {
                ...
                //}
                m_PauseResponseReceived = true;
                m_PauseDateField = dateParsed;
        ...
        protected void AdjustGeneral2(string recordingToken, TransportProtocol protocol)
        {
          StreamSetup streamSetup = new StreamSetup();
          streamSetup.Transport = new Transport();
          streamSetup.Transport.Protocol = protocol;
          ...
          VideoUtils.AdjustGeneral2(NewGenVideo, _username, _password, _messageTimeout,
                                 streamSetup.Transport.Protocol, streamSetup.Stream,
                                 replayUri);
        }

        protected void ReplaySequence2(
            string setupFields, string playFields, string pauseFields,
            Action<Action, Action, Action> actionControl,
            bool useVideo, bool useAudio, bool useMeta)
        {
          if (useVideo)
          {
            VideoEncoderConfiguration conf = new VideoEncoderConfiguration();
            conf.Encoding = VideoEncoding.JPEG;
            conf.Resolution = new VideoResolution();
            conf.Resolution.Width = 320;
            conf.Resolution.Height = 240;
            VideoUtils.AdjustVideo2(NewGenVideo, conf);
          }
          if (useAudio)
          {
            AudioEncoderConfiguration conf = new AudioEncoderConfiguration();
            conf.Encoding = AudioEncoding.G711;
            VideoUtils.AdjustAudio2(NewGenVideo, conf);
          }
          if (useMeta)
          {
            MetadataConfiguration conf = new MetadataConfiguration();
            VideoUtils.AdjustMeta2(NewGenVideo, conf);
          }
          NewGenVideo.EventSink = this;
          NewGenVideo.SetSequence(5, OperationDelay, false, false, false, setupFields, playFields, pauseFields);
          //NewGenVideo.SilentRun();
          NewGenVideo.Replay(actionControl);
        ...
        protected void MediaPlaybackTest2(TransportProtocol protocol, bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            RecordingInformation recInfo = GetRecordingParameters();

            AdjustGeneral2(recInfo.RecordingToken, protocol);
        ...
        protected void SimplePlaybackTest2(bool reverse, TransportProtocol protocol, bool useVideo, bool useAudio, bool useMeta)
        {
          _handleLogMessage = (message) =>
          {
            RtpPacket rtpPacket;
        ...
        protected void SimpleVideoPlaybackTest2(bool reverse, TransportProtocol protocol)
        {
          SimplePlaybackTest2(reverse, protocol, true, false, false);
        }
        protected void SimpleAudioPlaybackTest2(bool reverse, TransportProtocol protocol)
        {
          SimplePlaybackTest2(reverse, protocol, false, true, false);
        }
        protected void SimpleMetaPlaybackTest2(bool reverse, TransportProtocol protocol)
        {
          SimplePlaybackTest2(reverse, protocol, false, false, true);
        }
        protected void SimpleAllPlaybackTest2(bool reverse, TransportProtocol protocol)
        {
          bool metadataSupported = Features.ContainsFeature(Feature.MetadataRecording);
          bool audioSupported = Features.ContainsFeature(Feature.AudioRecording);
          SimplePlaybackTest2(reverse, protocol, true, audioSupported, metadataSupported);
        }
        ...
        protected void ReplayPauseTest(bool withRange, bool useVideo, bool useAudio, bool useMeta)
        {
            RunTest(() =>
            {
                ...
                int delta = 8;
                var lastReceivedPacketTime = DateTime.MinValue;

                bool initialWaitingFinished = false;
                bool afterInitialWaitingFinished = false;
                var locker = new object();

                m_PauseResponseReceived = false;
                m_PauseDateField = DateTime.MinValue;
                ...
                _handleLogMessage = (message) =>
                    {
                        lock (locker)
                        {
                            ...
                            RtpPacket packet;
                            if (HadleMessagePacket(message, out packet))
                            {
                                if (m_PauseResponseReceived)
                                {
                                    if (DateTime.MinValue != m_PauseDateField)
                                        return m_PauseDateField >= packet.TimeStamp;
                                    else
                                    {
                                        //Return false in the following 2 cases:
                                        //1. we haven't got out of initial 3 sec waiting 
                                        //2. we've got out of initial 3 second waiting and also don't wait 3 sec after last received packet
                                        return !initialWaitingFinished || afterInitialWaitingFinished;
                                    }
                                }
                ...
                //var initialWaitTimer = new System.Threading.Timer();
                ReplaySequence(REQUIRE_ONVIF_REPLAY + CRLF,
                               MakeFieldsSet(new string[] { REQUIRE_ONVIF_REPLAY, RangeClock(recInfo.EarliestRecording) })
                             + MakeFieldsSet(new string[] { REQUIRE_ONVIF_REPLAY }),
                               ...
                               (actionPlay, actionPause, actionTeardown) =>
                               {
                                   ...
                                   var afterInitialWaitingTimeout = 0;
                                   lock (locker)
                                   {
                                       initialWaitingFinished = true;
                                       //If there is no Date field in PAUSE-response
                                       if (m_PauseDateField == DateTime.MinValue)
                                           afterInitialWaitingTimeout = 3000 - (DateTime.UtcNow - lastReceivedPacketTime).Milliseconds;
                                   }

                                   if (afterInitialWaitingTimeout > 0)
                                   {
                                       Sleep(afterInitialWaitingTimeout);

                                       lock (locker)
                                       { afterInitialWaitingFinished = true; }
                                   }
        ...
        protected void ReplayPauseTest2(bool withRange, bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            RecordingInformation recInfo = GetRecordingParameters();
            ...
            //var initialWaitTimer = new System.Threading.Timer();
            ReplaySequence2(REQUIRE_ONVIF_REPLAY + CRLF,
                           MakeFieldsSet(new string[] { REQUIRE_ONVIF_REPLAY, RangeClock(recInfo.EarliestRecording) })
                         + MakeFieldsSet(new string[] { REQUIRE_ONVIF_REPLAY }),
                           (withRange ? MakeFieldsSet(new string[] { RangeClockExact(recInfo.EarliestRecording.AddSeconds(delta)) }) : ""),
                           (actionPlay, actionPause, actionTeardown) =>
        ...
        protected void ReplayStopOfPlayingTest2(bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            RecordingInformation recInfo = GetRecordingParameters();
        ...
        protected void ReplayIFramesTest2(bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            RecordingInformation recInfo = GetRecordingParameters();
        ...
        protected void ReplayRateControlTest2(bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            RecordingInformation recInfo = GetRecordingParameters();
        ...
        protected void ReplayImmediateHeaderTest2(bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            RecordingInformation recInfo = GetRecordingParameters();
        ...
        protected void ReplaySeekTest(bool useVideo, bool useAudio, bool useMeta)
        {
            RunTest(() =>
            {
                try
                {
                    ...
                    InternalLogger.GetInstance().LogMessage(string.Format("ReplaySeekTest started(useVideo = {0}, useAudio = {1}, useMeta = {2})", useVideo, useAudio, useMeta));

                    RecordingInformation recInfo = GetRecordingParameters();

                    AdjustVideo(recInfo.RecordingToken, TransportProtocol.UDP);

                    var firstFrameAfterPlayIsReceived = new ManualResetEvent(false);
                    bool waitForFirstFrame = false;
                    DateTime? playResponseTimeStamp = null;
                    var currentStartTime = DateTime.Now;

                    const int timeStampTolerance = 2000;

                    string goodPacketLogMsg = "";

                    var sendSignalGoodPacketIsReceived = new Action<RtpPacket>((receivedPacked) =>
                    {
                        InternalLogger.GetInstance().LogMessage("Sending signal \"GoodPacket is received\"");
                        goodPacketLogMsg = receivedPacked.ToString();
                        firstFrameAfterPlayIsReceived.Set();
                        waitForFirstFrame = false;
                    });

                    var checkOnlyMetaPacket = new Action<RtpPacket>((receivedPacked) =>
                    {
                        var start = currentStartTime.Subtract(new TimeSpan(0, 0, 0, 0, timeStampTolerance));
                        var end = currentStartTime.Add(new TimeSpan(1, 0, 0, 0));
                        if (recInfo.LatestRecordingSpecified)
                            end = recInfo.LatestRecording.Add(new TimeSpan(0, 0, 0, 0, timeStampTolerance));
                        if (start <= receivedPacked.TimeStamp && receivedPacked.TimeStamp <= end)
                            sendSignalGoodPacketIsReceived(receivedPacked);
                    });

                    _handleLogMessage = (message) =>
                    {
                        //InternalLogger.GetInstance().SwitchOnForCurrentThread();
                        RtpPacket receivedPacked = null;
                        if (HadleMessagePacket(message, out receivedPacked))
                        {
                            if (waitForFirstFrame)
                            {
                                InternalLogger.GetInstance().LogMessage(string.Format("Wait active. Received RTP packet with timestamp {0}", receivedPacked.TimeStamp));

                                InternalLogger.GetInstance().LogMessage(string.Format("Packet timestamp difference: {0}", receivedPacked.TimeStamp.Subtract(currentStartTime).TotalMilliseconds));
                              //if ((useAudio || useVideo) && Math.Abs(receivedPacked.TimeStamp.Subtract(currentStartTime).TotalMilliseconds) <= timeStampTolerance)
                                if ((useAudio || useVideo) && (receivedPacked.TimeStamp.Subtract(currentStartTime).TotalMilliseconds) >= -timeStampTolerance)
                                    sendSignalGoodPacketIsReceived(receivedPacked);
                                else if (useMeta)
                                    checkOnlyMetaPacket(receivedPacked);
                            }
                    ...
                    int delta = 5;
                    DateTime middleT1 = recInfo.EarliestRecording.AddSeconds(30);
                    ...
                    ReplaySequence(REQUIRE_ONVIF_REPLAY + CRLF,
                                 ...
                                 + MakeFieldsSet(new string[] 
                                   ...
                                   (actionPlay, actionPause, actionTeardown) =>
                                   {
                                       Action<DateTime> startFirstFrameWaiting = (T) =>
                                       {
                                           RunStep(() => LogStepEvent(RangeClock(T)), "Setting Range Clock Values");
                                           ClearPackets();

                                           currentStartTime = T;
                                           firstFrameAfterPlayIsReceived.Reset();
                                           waitForFirstFrame = true;
                                           InternalLogger.GetInstance().LogMessage(string.Format("PLAY command is sent with start time {0}. Wait for response. Time: {1}", currentStartTime, DateTime.Now));
                                       };

                                       var waitTimeout = Math.Max(2000, _messageTimeout);
                                       Action afterReceivingPlayResponse = () =>
                                       {
                                           playResponseTimeStamp = DateTime.Now;
                                           InternalLogger.GetInstance().LogMessage(string.Format("Response to PLAY command is received: {0}.", playResponseTimeStamp.Value));
                                           RunStep(() =>
                                           {
                                               if (!firstFrameAfterPlayIsReceived.WaitOne(waitTimeout))
                                                   throw new VideoException("Error: no frames with expected Timestamp after PLAY request within timeout");
                                               LogStepEvent(goodPacketLogMsg);
                                           },
                                                   "Timestamp check in first frame and PLAY request");
                                           ShowLastPackets();
                                       };

                                       startFirstFrameWaiting(recInfo.EarliestRecording);
                                       actionPlay();
                                       afterReceivingPlayResponse();

                                       startFirstFrameWaiting(middleT1);
                                       actionPlay();
                                       afterReceivingPlayResponse();

                                       startFirstFrameWaiting(middleT1.AddSeconds(-delta));
                                       actionPlay();
                                       afterReceivingPlayResponse();
        ...
        protected void ReplaySeekTest2(bool useVideo, bool useAudio, bool useMeta)
        {
          RunTest(() =>
          {
            try
            {
              ...
              const int timeStampTolerance = 2000;
              double realtimeStampTolerance = 0;
              ...
              _handleLogMessage = (message) =>
              {
                ...
                if (HadleMessagePacket(message, out receivedPacked))
                {
                  if (waitForFirstFrame)
                  {
                    ...
                    InternalLogger.GetInstance().LogMessage(string.Format("Packet timestamp difference: {0}", receivedPacked.TimeStamp.Subtract(currentStartTime).TotalMilliseconds));
                    //if ((useAudio || useVideo) && Math.Abs(receivedPacked.TimeStamp.Subtract(currentStartTime).TotalMilliseconds) <= timeStampTolerance)
                    if ((useAudio || useVideo) && (realtimeStampTolerance = receivedPacked.TimeStamp.Subtract(currentStartTime).TotalMilliseconds) >= -timeStampTolerance)
                      sendSignalGoodPacketIsReceived(receivedPacked);
                    else if (useMeta)
                      checkOnlyMetaPacket(receivedPacked);
                  }
                           ...
                           + MakeFieldsSet(new string[] 
                             ...
                             (actionPlay, actionPause, actionTeardown) =>
                             {
                               ...
                               Action afterReceivingPlayResponse = () =>
                               {
                                 ...
                                 RunStep(() =>
                                 {
                                   if (!firstFrameAfterPlayIsReceived.WaitOne(2000))
                                   //if (!firstFrameAfterPlayIsReceived.WaitOne(waitTimeout)) // outdated mechanics
                                       throw new VideoException("Error: no frames with expected Timestamp after PLAY request within timeout");
                                   LogStepEvent(goodPacketLogMsg);
                                 },
                                         "Timestamp check in first frame and PLAY request");
        ...
        {
          StepFailed(new VideoException(Message));
        }
