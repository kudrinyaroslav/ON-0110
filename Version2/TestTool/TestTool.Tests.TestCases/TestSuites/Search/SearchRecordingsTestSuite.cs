...
namespace TestTool.Tests.TestCases
{
    ...
    public partial class SearchRecordingsTestSuite : Base.SearchTest
    {
        ...
        [Test(Name = "GET RECORDING SEARCH RESULTS AND GET RECORDINGS CONSISTENCY",
           ...
           FunctionalityUnderTest = new Functionality[] { Functionality.FindRecordings, Functionality.GetRecordingSearchResults, Functionality.GetRecordings })]
            ...
            RunTest(() =>
                    {
                        ...
                        searchToken = FindRecordings(new SearchScope(), null, keepAlive);
                        ...
                        foreach (GetRecordingsResponseItem item in recordings)
                        {
                            ...
                            if (!foundRecordings.Any(RI => RI.RecordingToken == token))
                            {
                                logger.AppendFormat("Recording with token {0} not found within recordings got via GetRecordingSearchResults{1}", 
                        ...
                        bool localOk = CheckRecordings(recordings, foundRecordings, logger, true, false, true);
                        ok = ok && localOk;
        ...
        [Test(Name = "GET RECORDING SEARCH RESULTS AND GET RECORDING INFORMATION CONSISTENCY",
           Path = PATH,
           Order = "02.01.18",
           Id = "2-1-18",
           Category = Category.SEARCH,
           ...
           RequirementLevel = RequirementLevel.Must,
           FunctionalityUnderTest = new Functionality[] { Functionality.FindRecordings, Functionality.GetRecordingSearchResults, Functionality.GetRecordingInformation })]
        public void GetRecordingInformationAndSearchResultsConsistencyTest()
        {
            int testKeepAlive = _searchKeepAlive;
            SearchState state = SearchState.Completed;
            string searchToken = string.Empty;
            RunTest(() =>
                    {
                        ...
                        // check that all tokens from full list are present in list of found recordings
                        CompareLists(foundRecordings, foundRecordings.Select(e => GetRecordingInformation(e.RecordingToken)).ToArray());
                    },
                    () =>
                    {
                        if (state != SearchState.Completed)
