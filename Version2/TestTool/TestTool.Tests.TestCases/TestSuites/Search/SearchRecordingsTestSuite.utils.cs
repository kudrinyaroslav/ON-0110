...
namespace TestTool.Tests.TestCases
{
    public partial class SearchRecordingsTestSuite
    {
        ...
        protected void ValidateFoundRecordingsTokens(IEnumerable<RecordingInformation> recordings)
        {
            ...
            Assert(tokensOk, logger.ToStringTrimNewLine(), stepName);
        ...
        protected bool CheckRecordings(GetRecordingsResponseItem[] fullList, IEnumerable<RecordingInformation> list, StringBuilder logger,
                                       bool compareContent = true, bool compareTracks = true, bool compareSource = true)
        {
            bool ok = true;
            ...
            foreach (RecordingInformation info in list)
            {
                ...
                GetRecordingsResponseItem item = fullList.FirstOrDefault(RI => RI.RecordingToken == token);

                // Content [Description]
                if (compareContent && info.Content != item.Configuration.Content)
                {
                    ok = false;
                    logger.AppendFormat("Content is different for item with token='{0}' ('{1}' for GetRecordings and '{2}' for GetRecordingSearchResults){3}",
                                        token, item.Configuration.Content, info.Content, Environment.NewLine);
                }

                // TrackInformation 
                if (compareTracks)
                    ok = ok && CompareTracks(item.Tracks, info.Track, item.RecordingToken, logger);


                // RecordingSourceInformation 
                if (compareSource)
                    ok = ok && CompareSourceInformation(item.Configuration.Source, info.Source, item.RecordingToken, logger);
            }
