...
namespace TestTool.Tests.TestCases.Utils.Comparison
{
    ...
    {
        public static bool ValidateTokens<T>(IEnumerable<T> list, Func<T, 
                                             ...
                                             StringBuilder logger)
        {
            ...
            foreach (T item in list)
            {
                ...
                if (list.Count(I => tokenSelector(I) == token) != 1)
                {
                    tokensOk = false;
        ...
        public static bool CheckTokensInclusion<T1, T2>(IEnumerable<T1> fullList,
                                                        Func<T1, string> tokenSelector1,
                                                        ...
                                                        StringBuilder logger)
        {
            ...
            foreach (T2 item in shortList)
            {
                ...
                if (!fullList.Any(I => tokenSelector1(I) == token))
                {
                    logger.AppendFormat("{0} with token '{1}' not found in {2}{3}", itemName, token, fullListDescription, Environment.NewLine);
                    ok = false;
                }
            }
