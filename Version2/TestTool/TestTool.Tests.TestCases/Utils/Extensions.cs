...
namespace TestTool.Tests.TestCases
{
    ...
    static class Extensions
    {
        ...
        public static string GetNonMatchingAlphabeticalString(this IEnumerable<string> strings, int? outSize = null)
        {
            //empty set in input
            if (null == strings || !strings.Any())
                return outSize.HasValue ? new string('X', outSize.Value) : "X";
            ...
            if (null == outSize)
            {
                ...
                for (int size = 1; size <= maxSize; ++size)
                {
                    ...
                    for (int p = 0; p < 16*size; p++)
                    {
                        var r = RandomAlphabeticalString(size);
                        if (!stringsSet.Contains(r))
                            return r;
                    }
                }
            ...
            else
            {
                ...
                {
                    var r = RandomAlphabeticalString(outSize.Value);
                    if (!stringsSet.Contains(r))
                        return r;
                }

                //All attemts to generate new string was failed. Probably, it means there is no chance to generate
                //new string of size outSize
                return string.Empty;
            }
        }
        ...
        {
            return XmlConvert.ToTimeSpan(duration).TotalSeconds;
        ...
        public static bool tryConvertXmlBoolean(this string valueStr, out bool value, Action<string> logAction = null)
        {
            value = false;
            try
            { value = XmlConvert.ToBoolean(valueStr); }
            catch (Exception e)
            {
                if (null != logAction)
                    logAction(valueStr);
        ...
        public static bool tryConvertXmlBoolean(this string valueStr, Action<string> logAction = null)
        {
            var value = false;
            return tryConvertXmlBoolean(valueStr, out value, logAction);
        }
        ...
        public static System.Net.IPAddress GetBroadcastAddress(this System.Net.IPAddress address, System.Net.IPAddress subnetMask)
        {
